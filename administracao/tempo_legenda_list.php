<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("tempo_legenda_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Tempo Legenda: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="tempo_legenda_operation.php?back=<?=$link_back;?>">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      ...
      <tr>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_tempo_legenda"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center"><a href="tempo_legenda_view.php?id=<?=$vobj[$i]["id_tempo_legenda"]?>&back=<?=$link_back;?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_tempo_legenda"]?>" /></a>&nbsp;<a href="tempo_legenda_edit.php?id=<?=$vobj[$i]["id_tempo_legenda"]?>&back=<?=$link_back;?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_tempo_legenda"]?>" width="18" height="18" border="0"></a><a href="tempo_legenda_remove.php?id=<?=$vobj[$i]["id_tempo_legenda"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_tempo_legenda"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
