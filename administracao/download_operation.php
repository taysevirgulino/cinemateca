			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						DownloadManage::excluirDownload($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new Download();
						if(!$obj_atual->buscarDownload(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new Download();
						if(!$obj_alterar->buscarDownload(1, $itens[1])){ System::Redirect($link_list); }
				...
				case "status" : {
					for ($i=0; $i < count($itens); $i++){
						$obj = new Download();
						if($obj->buscarDownload(1, $itens[$i])){
