<?
	...
	$vobj = CurriculoSegmentoManage::consultarCurriculoSegmento(3, SearchMounter::MounterByQueryComparerOrder(CurriculoSegmentoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados CurriculoSegmento")
								 ->setSubject("Dados CurriculoSegmento")
								 ->setDescription("Dados CurriculoSegmento")
								 ->setKeywords("Dados CurriculoSegmento")
								 ->setCategory("CurriculoSegmento");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Currículo Segmento"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_curriculo_segmento"]));
	...
	$fileName = "CurriculoSegmento_".date("d-m-Y_H-i-s").".xlsx";
