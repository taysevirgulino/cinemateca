<?
	...
	$vobj = UsuarioManage::consultarUsuario(3, SearchMounter::MounterByQueryComparerOrder(UsuarioAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Usuario")
								 ->setSubject("Dados Usuario")
								 ->setDescription("Dados Usuario")
								 ->setKeywords("Dados Usuario")
								 ->setCategory("Usuario");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Usuário"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Usuário Perfil"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("F1", utf8_encode("Imagem"));
	$objPHPExcel->getActiveSheet()->getStyle("F1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("F")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Login"));
	$objPHPExcel->getActiveSheet()->getStyle("G1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("G")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("H1", utf8_encode("Senha"));
	$objPHPExcel->getActiveSheet()->getStyle("H1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("H")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("I1", utf8_encode("Data/Hora Cadastro"));
	$objPHPExcel->getActiveSheet()->getStyle("I1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("I")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("J1", utf8_encode("Data/Hora Edição"));
	$objPHPExcel->getActiveSheet()->getStyle("J1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("J")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("K1", utf8_encode("Data/Hora Login"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_usuario"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueUsuarioPerfil = UsuarioPerfilManage::buscarUsuarioPerfil(1, $vobj[$i]["id_usuario_perfil"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueUsuarioPerfil["titulo"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("F".($i+2), utf8_encode($vobj[$i]["imagem"]));
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($vobj[$i]["login"]));
		$objPHPExcel->getActiveSheet()->setCellValue("H".($i+2), utf8_encode($vobj[$i]["senha"]));
		$objPHPExcel->getActiveSheet()->setCellValue("I".($i+2), utf8_encode(System::FormatarData($vobj[$i]["datahora_cadastro"], "d/m/Y H:i:s")));
		$objPHPExcel->getActiveSheet()->setCellValue("J".($i+2), utf8_encode(System::FormatarData($vobj[$i]["datahora_edicao"], "d/m/Y H:i:s")));
		$objPHPExcel->getActiveSheet()->setCellValue("K".($i+2), utf8_encode(System::FormatarData($vobj[$i]["datahora_login"], "d/m/Y H:i:s")));
	...
	$fileName = "Usuario_".date("d-m-Y_H-i-s").".xlsx";
