<?
	...
	$vobj = AssociadoPlanoManage::consultarAssociadoPlano(3, SearchMounter::MounterByQueryComparerOrder(AssociadoPlanoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AssociadoPlano")
								 ->setSubject("Dados AssociadoPlano")
								 ->setDescription("Dados AssociadoPlano")
								 ->setKeywords("Dados AssociadoPlano")
								 ->setCategory("AssociadoPlano");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Associado Plano"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Valor"));
	$objPHPExcel->getActiveSheet()->getStyle("E1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("E")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("F1", utf8_encode("Prorata"));
	$objPHPExcel->getActiveSheet()->getStyle("F1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("F")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Recorrencia"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_associado_plano"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode(System::_TextByHtml($vobj[$i]["descricao"])));
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode($vobj[$i]["valor"]));
		$objPHPExcel->getActiveSheet()->setCellValue("F".($i+2), utf8_encode($vobj[$i]["prorata"]));
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($vobj[$i]["recorrencia"]));
	...
	$fileName = "AssociadoPlano_".date("d-m-Y_H-i-s").".xlsx";
