<?
	$link_add = "video_add.php";
	$link_edit = "video_edit.php";
	$link_remove = "video_remove.php";
	$link_list = "video_list.php";
	...
	$SearchQueryComparerCollection[] = new SearchQueryComparer(VideoAttribute::IdeOrigem(), SearchComparer::Igual(), SearchCondition::E(), Current::IdeOrigem());
	if(empty($busca_campo)){
		$busca_campo = VideoAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(VideoAttribute::Datahora(), SearchOrder::Decrescente());
		$ordenacao_campo = VideoAttribute::Datahora();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("video_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = VideoManage::count( SearchMounter::MounterByQueryCount(VideoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = VideoManage::consultar(SearchMounter::MounterByQueryLimit(VideoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = VideoAttribute::_GetAllAttributes();
?>