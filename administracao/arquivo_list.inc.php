<?
	$link_add = "arquivo_add.php";
	$link_edit = "arquivo_edit.php";
	$link_remove = "arquivo_remove.php";
	$link_list = "arquivo_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = ArquivoAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(ArquivoAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = ArquivoAttribute::Titulo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("arquivo_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = ArquivoManage::count( SearchMounter::MounterByQueryCount(ArquivoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = ArquivoManage::consultar(SearchMounter::MounterByQueryLimit(ArquivoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = ArquivoAttribute::_GetAllAttributes();
?>