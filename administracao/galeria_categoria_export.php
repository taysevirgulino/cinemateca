<?
	...
	$vobj = GaleriaCategoriaManage::consultarGaleriaCategoria(3, SearchMounter::MounterByQueryComparerOrder(GaleriaCategoriaAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados GaleriaCategoria")
								 ->setSubject("Dados GaleriaCategoria")
								 ->setDescription("Dados GaleriaCategoria")
								 ->setKeywords("Dados GaleriaCategoria")
								 ->setCategory("GaleriaCategoria");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Galeria Categoria"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Galeria Categoria Pai"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Prioridade"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_galeria_categoria"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueGaleriaCategoria = GaleriaCategoriaManage::buscarGaleriaCategoria(1, $vobj[$i]["id_galeria_categoria"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueGaleriaCategoria["nome"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($vobj[$i]["prioridade"]));
		$objPHPExcel->getActiveSheet()->setCellValue("H".($i+2), utf8_encode((($vobj[$i]["status"]==1) ? "Ativo" : "Inativo")));
	}

	$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 1);

	$fileName = "GaleriaCategoria_".date("d-m-Y_H-i-s").".xlsx";
