<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("pagina_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Página: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onSubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr<? if(empty($frm_pagina)){ ?> style="display:none"<? } ?>>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdPaginaPai">&nbsp;Sub-página de: </td>
        ...
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<? if(!empty($frm_imagem)){ ?><img src="../images/pagina/<?=$frm_imagem?>" style="display:block; margin-bottom:5px;" /><? } ?>
        ...
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
            <label><input onClick="_ViewSubPagina()" name="FrmFilhosExibir" type="radio" <? if($frm_filhos_exibir==1){ print('checked="checked"'); } ?> value="1" />Sim</label>
          	<label><input onClick="_ViewSubPagina()" name="FrmFilhosExibir" type="radio" <? if($frm_filhos_exibir==0){ print('checked="checked"'); } ?> value="0" />Não</label>
          </td>
        </tr>
