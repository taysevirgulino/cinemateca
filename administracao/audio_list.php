<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("audio_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Áudio: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="audio_operation.php?back=<?=$link_back;?>">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td width="15%" height="20"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_audio_categoria&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Áudio Categoria">Categoria</a></td>
        <td width="40%" height="20" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=titulo&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Título">Título</a></td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = AudioCategoriaManage::buscarAudioCategoria(1, $vobj[$i]["id_audio_categoria"]);	print $value["nome"]; ?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_audio"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center"><a href="<?=Url::Audio($vobj[$i]["id_audio"], $vobj[$i]["identificador"], $vobj[$i]["titulo"])?>" target="_blank"><img src="images/icones/world_link.gif" border="0" alt="Visualizar no Site" /></a>&nbsp;<a href="audio_view.php?id=<?=$vobj[$i]["id_audio"]?>&back=<?=$link_back;?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" alt="Visualizar | Código: <?=$vobj[$i]["id_audio"]?>" /></a>&nbsp;<a href="audio_edit.php?id=<?=$vobj[$i]["id_audio"]?>&back=<?=$link_back;?>"><img src="images/botao/Edit.gif" alt="Editar | Código: <?=$vobj[$i]["id_audio"]?>" width="18" height="18" border="0"></a><a href="audio_remove.php?id=<?=$vobj[$i]["id_audio"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" alt="Excluir | Código: <?=$vobj[$i]["id_audio"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
