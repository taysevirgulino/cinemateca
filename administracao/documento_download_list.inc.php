<?
	$link_add = "documento_download_add.php";
	$link_edit = "documento_download_edit.php";
	$link_remove = "documento_download_remove.php";
	$link_list = "documento_download_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = DocumentoDownloadAttribute::Datahora();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(DocumentoDownloadAttribute::Datahora(), SearchOrder::Crescente());
		$ordenacao_campo = DocumentoDownloadAttribute::Datahora();
	...
	if(!Validacao::isVazio($exportar)){
		...
		$export = array();
		$export["Object"] = "DocumentoDownload";
		...
		System::Redirect("documento_download_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = DocumentoDownloadManage::count( SearchMounter::MounterByQueryCount(DocumentoDownloadAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = DocumentoDownloadManage::consultar(SearchMounter::MounterByQueryLimit(DocumentoDownloadAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = DocumentoDownloadAttribute::_GetAllAttributes();
?>