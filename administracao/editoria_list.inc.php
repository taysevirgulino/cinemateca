<?
	$link_add = "editoria_add.php";
	$link_edit = "editoria_edit.php";
	$link_remove = "editoria_remove.php";
	$link_list = "editoria_list.php";
	...
	//$SearchQueryComparerCollection[] = new SearchQueryComparer(EditoriaAttribute::IdeOrigem(), SearchComparer::Igual(), SearchCondition::E(), Current::IdeOrigem());
	if(empty($busca_campo)){
		$busca_campo = EditoriaAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(EditoriaAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = EditoriaAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("editoria_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = EditoriaManage::count( SearchMounter::MounterByQueryCount(EditoriaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = EditoriaManage::consultar(SearchMounter::MounterByQueryLimit(EditoriaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = EditoriaAttribute::_GetAllAttributes();
?>