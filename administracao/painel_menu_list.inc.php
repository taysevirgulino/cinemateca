<?
	$link_add = "painel_menu_add.php";
	$link_edit = "painel_menu_edit.php";
	$link_remove = "painel_menu_remove.php";
	$link_list = "painel_menu_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = PainelMenuAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(PainelMenuAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = PainelMenuAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("painel_menu_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = PainelMenuManage::count( SearchMounter::MounterByQueryCount(PainelMenuAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = PainelMenuManage::consultar(SearchMounter::MounterByQueryLimit(PainelMenuAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = PainelMenuAttribute::_GetAllAttributes();
?>