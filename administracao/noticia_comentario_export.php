<?
	...
	$vobj = NoticiaComentarioManage::consultarNoticiaComentario(3, SearchMounter::MounterByQueryComparerOrder(NoticiaComentarioAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados NoticiaComentario")
								 ->setSubject("Dados NoticiaComentario")
								 ->setDescription("Dados NoticiaComentario")
								 ->setKeywords("Dados NoticiaComentario")
								 ->setCategory("NoticiaComentario");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Notícia Comentário"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Notícia"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Ip"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_noticia_comentario"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueNoticia = NoticiaManage::buscarNoticia(1, $vobj[$i]["id_noticia"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueNoticia["titulo"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($vobj[$i]["ip"]));
	...
	$fileName = "NoticiaComentario_".date("d-m-Y_H-i-s").".xlsx";
