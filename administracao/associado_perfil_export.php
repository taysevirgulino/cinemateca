<?
	...
	$vobj = AssociadoPerfilManage::consultarAssociadoPerfil(3, SearchMounter::MounterByQueryComparerOrder(AssociadoPerfilAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AssociadoPerfil")
								 ->setSubject("Dados AssociadoPerfil")
								 ->setDescription("Dados AssociadoPerfil")
								 ->setKeywords("Dados AssociadoPerfil")
								 ->setCategory("AssociadoPerfil");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Associado Perfil"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_associado_perfil"]));
	...
	$fileName = "AssociadoPerfil_".date("d-m-Y_H-i-s").".xlsx";
