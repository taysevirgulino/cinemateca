<?
	$link_add = "loja_add.php";
	$link_edit = "loja_edit.php";
	$link_remove = "loja_remove.php";
	$link_list = "loja_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = LojaAttribute::Numero();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(LojaAttribute::Numero(), SearchOrder::Crescente());
		$ordenacao_campo = LojaAttribute::Numero();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("loja_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = LojaManage::count( SearchMounter::MounterByQueryCount(LojaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = LojaManage::consultar(SearchMounter::MounterByQueryLimit(LojaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = LojaAttribute::_GetAllAttributes();
?>