<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("emkt_destinatario_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Emkt Destinatário: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdEmktDisparo">&nbsp;Emkt Disparo: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdEmktDisparo" class="frm" id="FrmIdEmktDisparo" style="width:500px;">
    <? for($i=0; $i < count($VObjEmktDisparo); $i++){ ?>
    <option value="<?=$VObjEmktDisparo[$i][0]?>" <? if($VObjEmktDisparo[$i][0] == $frm_id_emkt_disparo){ print("selected"); }?>><?=$VObjEmktDisparo[$i]["agendamento"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
    ...
    <script language="javascript">document.getElementById("FrmIdEmktDisparo").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdEmktDisparo':{'l':' Emkt Disparo','t':'labelFrmIdEmktDisparo','r':true},
