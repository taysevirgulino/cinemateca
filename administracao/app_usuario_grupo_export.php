<?
	...
	$vobj = AppUsuarioGrupoManage::consultarAppUsuarioGrupo(3, SearchMounter::MounterByQueryComparerOrder(AppUsuarioGrupoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AppUsuarioGrupo")
								 ->setSubject("Dados AppUsuarioGrupo")
								 ->setDescription("Dados AppUsuarioGrupo")
								 ->setKeywords("Dados AppUsuarioGrupo")
								 ->setCategory("AppUsuarioGrupo");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código App Usuário Grupo"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Sigla"));
	$objPHPExcel->getActiveSheet()->getStyle("D1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("D")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Tipo"));
	$objPHPExcel->getActiveSheet()->getStyle("E1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("E")->setAutoSize(true);


	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_app_usuario_grupo"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($vobj[$i]["sigla"]));
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode($vobj[$i]["tipo"]));
	}

	$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 1);

	$fileName = "AppUsuarioGrupo_".date("d-m-Y_H-i-s").".xlsx";
