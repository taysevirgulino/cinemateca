<?
	...
	$vobj = UsuarioPerfilManage::consultarUsuarioPerfil(3, SearchMounter::MounterByQueryComparerOrder(UsuarioPerfilAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados UsuarioPerfil")
								 ->setSubject("Dados UsuarioPerfil")
								 ->setDescription("Dados UsuarioPerfil")
								 ->setKeywords("Dados UsuarioPerfil")
								 ->setCategory("UsuarioPerfil");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Usuário Perfil"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Nivel"));
	$objPHPExcel->getActiveSheet()->getStyle("D1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("D")->setAutoSize(true);


	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_usuario_perfil"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($vobj[$i]["nivel"]));
	}

	$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 1);
	$objPHPExcel->getActiveSheet()->setAutoFilter( $objPHPExcel->getActiveSheet()->calculateWorksheetDimension() );

	$fileName = "UsuarioPerfil_".date("d-m-Y_H-i-s").".xlsx";
