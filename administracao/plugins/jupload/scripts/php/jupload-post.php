...
if ($files) {
	foreach ($files as $relativePath => $tempPath)  {
		...
		//DARLEY----------------------------------------------------------------------------|
		$Str = strtolower($relativePath);
		$Str = eregi_replace("[çÇ]+", "c", $Str);
		$Str = eregi_replace("[áÁäÄàÀãÃâÂ]+", "a", $Str);
		$Str = eregi_replace("[éÉëËèÈêÊ]+", "e", $Str);
		$Str = eregi_replace("[íÍïÏìÌîÎ]+", "i", $Str);
		$Str = eregi_replace("[óÓöÖòÒõÕôÔ]+", "o", $Str);
		$Str = eregi_replace("[úÚüÜùÙûÛ]+", "u", $Str);			
