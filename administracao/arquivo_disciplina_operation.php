			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						ArquivoDisciplinaManage::excluirArquivoDisciplina($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new ArquivoDisciplina();
						if(!$obj_atual->buscarArquivoDisciplina(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new ArquivoDisciplina();
						if(!$obj_alterar->buscarArquivoDisciplina(1, $itens[1])){ System::Redirect($link_list); }
				...
				case "status" : {
					for ($i=0; $i < count($itens); $i++){
						...
						if($obj->buscarArquivoDisciplina(1, $itens[$i])){
