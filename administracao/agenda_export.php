<?
	...
	$vobj = AgendaManage::consultarAgenda(3, SearchMounter::MounterByQueryComparerOrder(AgendaAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Agenda")
								 ->setSubject("Dados Agenda")
								 ->setDescription("Dados Agenda")
								 ->setKeywords("Dados Agenda")
								 ->setCategory("Agenda");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Agenda"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Agenda Categoria"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Hora"));
	$objPHPExcel->getActiveSheet()->getStyle("G1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("G")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("H1", utf8_encode("Local"));
	$objPHPExcel->getActiveSheet()->getStyle("H1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("H")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("I1", utf8_encode("Informações Contato"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_agenda"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueAgendaCategoria = AgendaCategoriaManage::buscarAgendaCategoria(1, $vobj[$i]["id_agenda_categoria"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueAgendaCategoria["nome"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($vobj[$i]["hora"]));
		$objPHPExcel->getActiveSheet()->setCellValue("H".($i+2), utf8_encode($vobj[$i]["local"]));
		$objPHPExcel->getActiveSheet()->setCellValue("I".($i+2), utf8_encode($vobj[$i]["informacoes_contato"]));
	...
	$fileName = "Agenda_".date("d-m-Y_H-i-s").".xlsx";
