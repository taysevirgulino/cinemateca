<?
	$link_add = "foto_add.php";
	$link_edit = "foto_edit.php";
	$link_remove = "foto_remove.php";
	$link_list = "foto_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = FotoAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(FotoAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = FotoAttribute::Titulo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("foto_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = FotoManage::count( SearchMounter::MounterByQueryCount(FotoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = FotoManage::consultar(SearchMounter::MounterByQueryLimit(FotoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = FotoAttribute::_GetAllAttributes();
?>