<?
	...
	$vobj = ArtigoManage::consultarArtigo(3, SearchMounter::MounterByQueryComparerOrder(ArtigoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Artigo")
								 ->setSubject("Dados Artigo")
								 ->setDescription("Dados Artigo")
								 ->setKeywords("Dados Artigo")
								 ->setCategory("Artigo");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Artigo"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Artigo Articulista"));
	$objPHPExcel->getActiveSheet()->getStyle("C1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("C")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Título"));
	$objPHPExcel->getActiveSheet()->getStyle("D1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("D")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Resumo"));
	$objPHPExcel->getActiveSheet()->getStyle("E1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("E")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("F1", utf8_encode("Texto"));
	$objPHPExcel->getActiveSheet()->getStyle("F1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("F")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Arquivo Anexo"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_artigo"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueArtigoArticulista = ArtigoArticulistaManage::buscarArtigoArticulista(1, $vobj[$i]["id_artigo_articulista"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueArtigoArticulista["nome"]));
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($vobj[$i]["titulo"]));
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode(System::_TextByHtml($vobj[$i]["resumo"])));
		$objPHPExcel->getActiveSheet()->setCellValue("F".($i+2), utf8_encode(System::_TextByHtml($vobj[$i]["texto"])));
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($vobj[$i]["arquivo_anexo"]));
	...
	$fileName = "Artigo_".date("d-m-Y_H-i-s").".xlsx";
