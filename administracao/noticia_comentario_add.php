<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("noticia_comentario_add.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Notícia Comentário: Cadastro </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_add?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdNoticia">&nbsp;Notícia: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdNoticia" class="frm" id="FrmIdNoticia" style="width:500px;">
    <? for($i=0; $i < count($VObjNoticia); $i++){ ?>
    <option value="<?=$VObjNoticia[$i][0]?>" <? if($VObjNoticia[$i][0] == $frm_id_noticia){ print("selected"); }?>><?=$VObjNoticia[$i]["titulo"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmIp" id="FrmIp" class="frm" size="15" maxlength="15" value="<?=$frm_ip?>">
          </td>
        </tr>
        <tr>
    ...
    <script language="javascript">document.getElementById("FrmIdNoticia").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdNoticia':{'l':' Notícia','t':'labelFrmIdNoticia','r':true},
