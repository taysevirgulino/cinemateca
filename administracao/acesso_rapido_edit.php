<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("acesso_rapido_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Acesso Rápido: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdAcessoRapidoPai">&nbsp;Menu: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdAcessoRapidoPai" class="frm" id="FrmIdAcessoRapidoPai" style="width:500px;">
              	<option value="" selected="selected"></option>
                <option value="0" <? if($frm_id_acesso_rapido_pai==0){ print("selected"); }?>>[Sem Menu]</option>
				<? for($i=0; $i < count($VObjAcessoRapido); $i++){ ?>
                <option value="<?=$VObjAcessoRapido[$i][1]?>" <? if($VObjAcessoRapido[$i][1] == $frm_id_acesso_rapido_pai){ print("selected"); }?>><?=$VObjAcessoRapido[$i][0]?></option>
                <? } ?>
              </select>
          </td>
        </tr>
        <tr id="trFrmIdAcessoRapidoBloco" style="display:none;">
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdAcessoRapidoBloco">&nbsp;Bloco: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdAcessoRapidoBloco" class="frm" id="FrmIdAcessoRapidoBloco" style="width:500px;">
				<? for($i=0; $i < count($VObjAcessoRapidoBloco); $i++){ ?>
                <option value="<?=$VObjAcessoRapidoBloco[$i]["id_acesso_rapido_bloco"]?>" <? if($VObjAcessoRapidoBloco[$i]["id_acesso_rapido_bloco"] == $frm_id_acesso_rapido_bloco){ print("selected"); }?>><?=$VObjAcessoRapidoBloco[$i]["nome"]?></option>
                <? } ?>
              </select>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<select name="FrmTarget" class="frm" id="FrmTarget"><option value="_blank" <? if($frm_target == '_blank'){print('selected="selected"');}?>>Nova P&aacute;gina</option><option value="_parent" <? if($frm_target == '_parent'){print('selected="selected"');}?>>Mesma P&aacute;gina</option></select>
          </td>
        </tr>
        <tr>
    ...
    <script language="javascript">document.getElementById("FrmIdAcessoRapidoPai").focus();</script>
	<script language="javascript">
    <!--
		function ShowIdAcessoRapidoBloco(valor){
			if(valor == "0"){
				$("#trFrmIdAcessoRapidoBloco").css("display", "table-row");
			}else{
				$("#trFrmIdAcessoRapidoBloco").css("display", "none");
			}
		}
		$(function(){
			$("#FrmIdAcessoRapidoPai").change(function(){
				ShowIdAcessoRapidoBloco($(this).val());
			});
		});
		ShowIdAcessoRapidoBloco("<?=$frm_id_acesso_rapido_pai?>");
		function _Validar(){
			return vFrm.exec();
		}
        var a_fields = {
             'FrmIdAcessoRapidoPai':{'l':' Menu','t':'labelFrmIdAcessoRapidoPai','r':true},
            'FrmIdAcessoRapidoBloco':{'l':' Bloco','t':'labelFrmIdAcessoRapidoBloco','r':true},
