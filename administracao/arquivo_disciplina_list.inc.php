<?
	$link_add = "arquivo_disciplina_add.php";
	$link_edit = "arquivo_disciplina_edit.php";
	$link_remove = "arquivo_disciplina_remove.php";
	$link_list = "arquivo_disciplina_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = ArquivoDisciplinaAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(ArquivoDisciplinaAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = ArquivoDisciplinaAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		$export = array();
		$export["Object"] = "ArquivoDisciplina";
		...
		System::Redirect("arquivo_disciplina_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = ArquivoDisciplinaManage::count( SearchMounter::MounterByQueryCount(ArquivoDisciplinaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = ArquivoDisciplinaManage::consultar(SearchMounter::MounterByQueryLimit(ArquivoDisciplinaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = ArquivoDisciplinaAttribute::_GetAllAttributes();
?>