<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("usuario_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Usuário: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="usuario_operation.php?back=<?=$link_back;?>">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td width="55%" height="20" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=nome&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Nome">Nome</a></td>
        <td width="15%" height="20" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=datahora_login&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Data/Hora Login">Login</a></td>
      ...
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left">
			<i><?	$value = UsuarioPerfilManage::buscarUsuarioPerfil(1, $vobj[$i]["id_usuario_perfil"]);	print $value["titulo"]; ?></i><br />
			<b><?=System::FormatarStringList($vobj[$i]["nome"])?></b><br>
			<?=System::FormatarStringList($vobj[$i]["email"])?>; <?=System::FormatarStringList($vobj[$i]["login"])?>
		</td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=System::FormatarData($vobj[$i]["datahora_login"], "d/m/y [Hhi]");?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=UsuarioStatus::_Descricao($vobj[$i]["status"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_usuario"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center"><a href="usuario_view.php?id=<?=$vobj[$i]["id_usuario"]?>&back=<?=$link_back;?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_usuario"]?>" /></a>&nbsp;<a href="usuario_edit.php?id=<?=$vobj[$i]["id_usuario"]?>&back=<?=$link_back;?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_usuario"]?>" width="18" height="18" border="0"></a><a href="usuario_remove.php?id=<?=$vobj[$i]["id_usuario"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_usuario"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
