<?
	$link_add = "jornal_estrutura_add.php";
	$link_edit = "jornal_estrutura_edit.php";
	$link_remove = "jornal_estrutura_remove.php";
	$link_list = "jornal_estrutura_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = JornalEstruturaAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(JornalEstruturaAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = JornalEstruturaAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("jornal_estrutura_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = JornalEstruturaManage::count( SearchMounter::MounterByQueryCount(JornalEstruturaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = JornalEstruturaManage::consultar(SearchMounter::MounterByQueryLimit(JornalEstruturaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = JornalEstruturaAttribute::_GetAllAttributes();
?>