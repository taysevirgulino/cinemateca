			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						AcessoRapidoManage::excluirAcessoRapido($itens[$i]);
					}
				...
				case "prioridade" : {
					if(count($itens) >= 2){
						...
						if(!$obj_atual->buscarAcessoRapido(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new AcessoRapido();
						if(!$obj_alterar->buscarAcessoRapido(1, $itens[1])){ System::Redirect($link_list); }
				...
				case "status" : {
					for ($i=0; $i < count($itens); $i++){
						...
						if($obj->buscarAcessoRapido(1, $itens[$i])){
