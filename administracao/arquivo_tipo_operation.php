			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						ArquivoTipoManage::excluirArquivoTipo($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new ArquivoTipo();
						if(!$obj_atual->buscarArquivoTipo(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new ArquivoTipo();
						if(!$obj_alterar->buscarArquivoTipo(1, $itens[1])){ System::Redirect($link_list); }
				...
				case "status" : {
					for ($i=0; $i < count($itens); $i++){
						$obj = new ArquivoTipo();
						if($obj->buscarArquivoTipo(1, $itens[$i])){
