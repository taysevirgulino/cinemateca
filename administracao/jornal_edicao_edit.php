<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("jornal_edicao_edit.inc.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Revista Edição: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
      ...
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmNumero" id="FrmNumero" class="frm" size="5" maxlength="5" value="<?=$frm_numero?>">
          </td>
        </tr>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmAno">&nbsp;Ano: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmAno" id="FrmAno" class="frm" size="4" maxlength="4" value="<?=$frm_ano?>">
          </td>
        </tr>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmDataInicial">&nbsp;Data Inicial: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmDataInicial" id="FrmDataInicial" class="frm" size="10" maxlength="10" value="<?=$frm_data_inicial?>">&nbsp;<input type="button" name="BTDataInicial" id="BTDataInicial" class="frm" value="..." ><script type="text/javascript"> Calendar.setup({ inputField:"FrmDataInicial", ifFormat:"%d/%m/%Y", showsTime:true, button:"BTDataInicial", singleClick:true, step:1 }); </script>&nbsp;<span class="frm_exemplo">Ex: <?=date("d/m/Y")?></span>
          </td>
        </tr>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmDataFinal">&nbsp;Data Final: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmDataFinal" id="FrmDataFinal" class="frm" size="10" maxlength="10" value="<?=$frm_data_final?>">&nbsp;<input type="button" name="BTDataFinal" id="BTDataFinal" class="frm" value="..." ><script type="text/javascript"> Calendar.setup({ inputField:"FrmDataFinal", ifFormat:"%d/%m/%Y", showsTime:true, button:"BTDataFinal", singleClick:true, step:1 }); </script>&nbsp;<span class="frm_exemplo">Ex: <?=date("d/m/Y")?></span>
          </td>
        </tr>
		<? 
			$campo = 0;
			for($i=0; $i < count($VObjJornalEstrutura); $i++){ 
		?>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto">&nbsp; <?=$VObjJornalEstrutura[$i]["nome"]?>: </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<?
				$Pagina = JornalPaginaManage2::PaginaByEdicaoEstrutura($ID, $VObjJornalEstrutura[$i]["id_jornal_estrutura"]);
            	if(!empty($Pagina)){
					print('<a href="jornal_pagina_remove.php?idj='.$ID.'&id='.$Pagina["id_jornal_pagina"].'" onclick="javascript: return confirm(\'Deseja realmente EXCLUIR?\');"><img src="images/botao/Delete.gif" alt="Excluir | Código: '.$Pagina["id_jornal_pagina"].'" width="18" height="18" border="0"></a>');
					print('<br /><img src="../images/jornal_pagina/A'.$Pagina["imagem"].'" />');
				}else{
					if($campo < 17){
						print('<input type="file" name="FrmImagemFile'.$VObjJornalEstrutura[$i]["id_jornal_estrutura"].'" id="FrmImagemFile'.$VObjJornalEstrutura[$i]["id_jornal_estrutura"].'" class="frm" style="width:80%;" size="110" maxlength="255" value="">');
						$campo++;
					}
				}
			?>
    ...
    <script language="javascript">document.getElementById("FrmNumero").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            ...
            'FrmAno':{'l':' Ano','t':'labelFrmAno','r':true},
