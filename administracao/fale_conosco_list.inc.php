<?
	...
	$SearchQueryComparerCollection[] = new SearchQueryComparer(FaleConoscoAttribute::IdeOrigem(), SearchComparer::Igual(), SearchCondition::E(), Current::IdeOrigem());
	if(empty($busca_campo)){
		$busca_campo = FaleConoscoAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(FaleConoscoAttribute::Status(), SearchOrder::Crescente());
		$SearchQueryOrderCollection[] = new SearchQueryOrder(FaleConoscoAttribute::Datahora(), SearchOrder::Decrescente());
		$ordenacao_campo = FaleConoscoAttribute::Status();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("fale_conosco_export.php?ide=$export_ide");
	}
	
	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = FaleConoscoManage::count( SearchMounter::MounterByQueryCount(FaleConoscoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = FaleConoscoManage::consultar(SearchMounter::MounterByQueryLimit(FaleConoscoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = FaleConoscoAttribute::_GetAllAttributes();
?>