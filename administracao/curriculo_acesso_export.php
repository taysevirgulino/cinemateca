<?
	...
	$vobj = CurriculoAcessoManage::consultarCurriculoAcesso(3, SearchMounter::MounterByQueryComparerOrder(CurriculoAcessoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados CurriculoAcesso")
								 ->setSubject("Dados CurriculoAcesso")
								 ->setDescription("Dados CurriculoAcesso")
								 ->setKeywords("Dados CurriculoAcesso")
								 ->setCategory("CurriculoAcesso");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Currículo Acesso"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Currículo"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_curriculo_acesso"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueCurriculo = CurriculoManage::buscarCurriculo(1, $vobj[$i]["id_curriculo"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueCurriculo["nome"]));
	...
	$fileName = "CurriculoAcesso_".date("d-m-Y_H-i-s").".xlsx";
