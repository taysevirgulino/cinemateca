<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("cronograma_etapa_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Cronograma Etapa: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="cronograma_etapa_operation.php?back=<?=$link_back;?>">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td width="30%" height="20"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_cronograma_tipo&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Cronograma Tipo">Tipo</a></td>
        <td width="30%" height="20" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=titulo&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Título">Título</a></td>
        <td width="10%" height="20" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=porcentagem&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Porcentagem">Porcentagem</a></td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = CronogramaTipoManage::buscarCronogramaTipo(1, $vobj[$i]["id_cronograma_tipo"]);	print $value["titulo"]; ?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i]["titulo"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=System::FormatarStringList($vobj[$i]["porcentagem"])?> %</td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_cronograma_etapa"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center"><a href="cronograma_etapa_priority.php?idatual=<?=$vobj[$i]["id_cronograma_etapa"]?>&idalterar=<?=$vobj[$i-$h]["id_cronograma_etapa"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente Alterar a Prioridade?');"><img src="images/seta1.gif" width="15" height="15" border="0" title="Alterar Prioridade | Código: <?=$vobj[$i]["id_cronograma_etapa"]?> por <?=$vobj[$i-$h]["id_cronograma_etapa"]?>" /></a>&nbsp;<a href="cronograma_etapa_view.php?id=<?=$vobj[$i]["id_cronograma_etapa"]?>&back=<?=$link_back;?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_cronograma_etapa"]?>" /></a>&nbsp;<a href="cronograma_etapa_edit.php?id=<?=$vobj[$i]["id_cronograma_etapa"]?>&back=<?=$link_back;?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_cronograma_etapa"]?>" width="18" height="18" border="0"></a><a href="cronograma_etapa_remove.php?id=<?=$vobj[$i]["id_cronograma_etapa"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_cronograma_etapa"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
