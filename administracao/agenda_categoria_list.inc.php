<?
	$link_add = "agenda_categoria_add.php";
	$link_edit = "agenda_categoria_edit.php";
	$link_remove = "agenda_categoria_remove.php";
	$link_list = "agenda_categoria_list.php";
	...
	$SearchQueryComparerCollection[] = new SearchQueryComparer(AgendaCategoriaAttribute::IdeOrigem(), SearchComparer::Igual(), SearchCondition::E(), Current::IdeOrigem());
	if(empty($busca_campo)){
		$busca_campo = AgendaCategoriaAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(AgendaCategoriaAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = AgendaCategoriaAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("agenda_categoria_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = AgendaCategoriaManage::count( SearchMounter::MounterByQueryCount(AgendaCategoriaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = AgendaCategoriaManage::consultar(SearchMounter::MounterByQueryLimit(AgendaCategoriaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = AgendaCategoriaAttribute::_GetAllAttributes();
?>