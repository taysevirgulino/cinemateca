<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("jornal_pagina_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Revista Pagina: Listagem </td>
  </tr>
  <tr>
      ...
      <table width="100%" border="0">
        ...
        <tr>
          <td colspan="4" align="right" height="25"><input name="pesquisar" type="submit" class="frm_botao" id="pesquisar" value="Pesquisar" />
            <input name="cancelar" type="button" class="frm_botao" id="cancelar" value="Cancelar" onclick="javascript:window.open('<?=$link_list?>', '_parent');" /></td>
        </tr>
		<tr>
  ...
  <tr>
    <td height="30" align="right"><?=$PG_LABEL?></td>
  </tr>
  <form id="frmOperacao" name="frmOperacao" method="post" action="jornal_pagina_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="33%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_jornal_edicao&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Revista Edição">Revista Edição</a></td>
        <td height="20" width="33%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_jornal_estrutura&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Revista Estrutura">Revista Estrutura</a></td>
        <td height="20" width="33%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=imagem&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Imagem">Imagem</a></td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = JornalEdicaoManage::buscarJornalEdicao(1, $vobj[$i]["id_jornal_edicao"]);	print $value["numero"]; ?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = JornalEstruturaManage::buscarJornalEstrutura(1, $vobj[$i]["id_jornal_estrutura"]);	print $value["nome"]; ?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i]["imagem"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_jornal_pagina"]?>" class="InputOperacao" /></td>
        <td height="25" bgcolor="#CCCCCC" align="center" style="padding:2px;"><a href="jornal_pagina_view.php?id=<?=$vobj[$i]["id_jornal_pagina"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" alt="Visualizar | Código: <?=$vobj[$i]["id_jornal_pagina"]?>" /></a>&nbsp;<a href="jornal_pagina_edit.php?id=<?=$vobj[$i]["id_jornal_pagina"]?>"><img src="images/botao/Edit.gif" alt="Editar | Código: <?=$vobj[$i]["id_jornal_pagina"]?>" width="18" height="18" border="0"></a><a href="jornal_pagina_remove.php?id=<?=$vobj[$i]["id_jornal_pagina"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" alt="Excluir | Código: <?=$vobj[$i]["id_jornal_pagina"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
