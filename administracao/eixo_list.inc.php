<?
	$link_add = "eixo_add.php";
	$link_edit = "eixo_edit.php";
	$link_remove = "eixo_remove.php";
	$link_list = "eixo_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = EixoAttribute::Categoria();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(EixoAttribute::Categoria(), SearchOrder::Crescente());
		$ordenacao_campo = EixoAttribute::Categoria();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("eixo_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = EixoManage::count( SearchMounter::MounterByQueryCount(EixoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = EixoManage::consultar(SearchMounter::MounterByQueryLimit(EixoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = EixoAttribute::_GetAllAttributes();
?>