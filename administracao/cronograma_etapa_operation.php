			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						CronogramaEtapaManage::excluirCronogramaEtapa($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new CronogramaEtapa();
						if(!$obj_atual->buscarCronogramaEtapa(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new CronogramaEtapa();
						if(!$obj_alterar->buscarCronogramaEtapa(1, $itens[1])){ System::Redirect($link_list); }
				...
				case "status" : {
					for ($i=0; $i < count($itens); $i++){
						...
						if($obj->buscarCronogramaEtapa(1, $itens[$i])){
