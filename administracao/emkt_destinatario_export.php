<?
	...
	$vobj = EmktDestinatarioManage::consultarEmktDestinatario(3, SearchMounter::MounterByQueryComparerOrder(EmktDestinatarioAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados EmktDestinatario")
								 ->setSubject("Dados EmktDestinatario")
								 ->setDescription("Dados EmktDestinatario")
								 ->setKeywords("Dados EmktDestinatario")
								 ->setCategory("EmktDestinatario");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Emkt Destinatário"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Emkt Disparo"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_emkt_destinatario"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueEmktDisparo = EmktDisparoManage::buscarEmktDisparo(1, $vobj[$i]["id_emkt_disparo"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueEmktDisparo["agendamento"]));
	...
	$fileName = "EmktDestinatario_".date("d-m-Y_H-i-s").".xlsx";
