<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("download_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Download: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdDownloadCategoria">&nbsp;Categoria: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdDownloadCategoria" class="frm" id="FrmIdDownloadCategoria" style="width:500px;">
    <? for($i=0; $i < count($VObjDownloadCategoria); $i++){ ?>
    <option value="<?=$VObjDownloadCategoria[$i]["id_download_categoria"]?>" <? if($VObjDownloadCategoria[$i]["id_download_categoria"] == $frm_id_download_categoria){ print("selected"); }?>><?=$VObjDownloadCategoria[$i]["nome"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmNome" id="FrmNome" class="frm" size="80" maxlength="200" value="<?=$frm_nome?>">
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmClick">&nbsp;Arquivo atual: </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<a href="../files/download/<?=$frm_arquivo?>" target="_blank" title="Clique para Baixar"><?=$frm_arquivo?></a>
          </td>
        </tr>
        <tr>
        ...
        <? if(!empty($frm_imagem)){ ?>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmFotoArquivo">&nbsp;Imagem atual:  </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<img src="../images/download/A<?=$frm_imagem?>" />
            </td>
        </tr>
        <? 
			}
    ...
    <script language="javascript">document.getElementById("FrmIdDownloadCategoria").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdDownloadCategoria':{'l':' Download Categoria','t':'labelFrmIdDownloadCategoria','r':true},
            'FrmNome':{'l':' Nome','t':'labelFrmNome','r':true},
            'FrmDescricao':{'l':' Descrição','t':'labelFrmDescricao'},
            'FrmArquivoFile':{'l':' Arquivo','t':'labelFrmArquivo','f':'file'},
