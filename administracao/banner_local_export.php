<?
	...
	$vobj = BannerLocalManage::consultarBannerLocal(3, SearchMounter::MounterByQueryComparerOrder(BannerLocalAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados BannerLocal")
								 ->setSubject("Dados BannerLocal")
								 ->setDescription("Dados BannerLocal")
								 ->setKeywords("Dados BannerLocal")
								 ->setCategory("BannerLocal");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Banner Local"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_banner_local"]));
	...
	$fileName = "BannerLocal_".date("d-m-Y_H-i-s").".xlsx";
