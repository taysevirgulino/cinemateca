...
class PagSeguro {
   ...
   *               produtos é preferivel inplantá-lo no new PagSeguro(array('item_peso_1' => 1300))
   * frete      => integer ou float (opcional) coloque o valor do frete, caso seja um frete único
   *               para todos os produtos é preferivel inplantá-lo no new PagSeguro(array('item_frete_1' => 30))
   *
   * @access public
   * @param array $item O elemento que será adicionado
   * @return object PagSeguro O próprio objeto para que possa ser concatenado a outro comando dele mesmo
   */
  ...
  function mostra ($args=array()) {
    ...
    $_form = array();
    if ($args['open_form'])
      $_form[] = '<form id="pagseguro" name="pagseguro" target="_parent" action="https://pagseguro.uol.com.br/security/webpagamentos/webpagto.aspx" method="post">';
    ...
    if ($args['show_submit']) {
      if ($args['img_button']) {
        $_form[] = sprintf('  <input type="image" src="%s" name="submit" title="Clique para PAGAR"  />', $args['img_button']);
