<?
	...
	$vobj = AcessoRapidoBlocoManage::consultarAcessoRapidoBloco(3, SearchMounter::MounterByQueryComparerOrder(AcessoRapidoBlocoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AcessoRapidoBloco")
								 ->setSubject("Dados AcessoRapidoBloco")
								 ->setDescription("Dados AcessoRapidoBloco")
								 ->setKeywords("Dados AcessoRapidoBloco")
								 ->setCategory("AcessoRapidoBloco");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Acesso Rápido Bloco"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_acesso_rapido_bloco"]));
	...
	$fileName = "AcessoRapidoBloco_".date("d-m-Y_H-i-s").".xlsx";
