<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("documento_download_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Documento Download: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdDocumento">&nbsp;Documento: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdDocumento" class="frm" id="FrmIdDocumento" style="width:500px;">
    <? for($i=0; $i < count($VObjDocumento); $i++){ ?>
    <option value="<?=$VObjDocumento[$i]["id_documento"]?>" <? if($VObjDocumento[$i]["id_documento"] == $frm_id_documento){ print("selected"); }?>><?=$VObjDocumento[$i]["titulo"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
    ...
    <script language="javascript">document.getElementById("FrmIdDocumento").focus();</script>
