<?
	...
	$vobj = MuralRecadoManage::consultarMuralRecado(3, SearchMounter::MounterByQueryComparerOrder(MuralRecadoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados MuralRecado")
								 ->setSubject("Dados MuralRecado")
								 ->setDescription("Dados MuralRecado")
								 ->setKeywords("Dados MuralRecado")
								 ->setCategory("MuralRecado");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Mural Recado"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_mural_recado"]));
	...
	$fileName = "MuralRecado_".date("d-m-Y_H-i-s").".xlsx";
