<?
	...
	$vobj = MensagemAcessoManage::consultarMensagemAcesso(3, SearchMounter::MounterByQueryComparerOrder(MensagemAcessoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados MensagemAcesso")
								 ->setSubject("Dados MensagemAcesso")
								 ->setDescription("Dados MensagemAcesso")
								 ->setKeywords("Dados MensagemAcesso")
								 ->setCategory("MensagemAcesso");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Mensagem Acesso"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_mensagem_acesso"]));
	...
	$fileName = "MensagemAcesso_".date("d-m-Y_H-i-s").".xlsx";
