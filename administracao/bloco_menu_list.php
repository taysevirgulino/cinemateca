<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("bloco_menu_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Bloco Menu: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="bloco_menu_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="30%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_bloco_menu_pai&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Bloco Menu">Menu</a></td>
        ...
        <td height="20" width="15%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=target&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Abrir">Abrir</a></td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><? $value = BlocoMenuManage::HerancaByString($vobj[$i]["id_bloco_menu_pai"]);	print ((!empty($value)) ? $value : "[nenhum]" ); ?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_bloco_menu"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="bloco_menu_priority.php?idatual=<?=$vobj[$i]["id_bloco_menu"]?>&idalterar=<?=$vobj[$i-$h]["id_bloco_menu"]?>" onclick="javascript: return confirm('Deseja realmente Alterar a Prioridade?');"><img src="images/seta1.gif" width="15" height="15" border="0" alt="Alterar Prioridade | Código: <?=$vobj[$i]["id_bloco_menu"]?> por <?=$vobj[$i-$h]["id_bloco_menu"]?>" /></a>&nbsp;<a href="bloco_menu_view.php?id=<?=$vobj[$i]["id_bloco_menu"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_bloco_menu"]?>" /></a>&nbsp;<a href="bloco_menu_edit.php?id=<?=$vobj[$i]["id_bloco_menu"]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_bloco_menu"]?>" width="18" height="18" border="0"></a><a href="bloco_menu_remove.php?id=<?=$vobj[$i]["id_bloco_menu"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_bloco_menu"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
