<?
	...
	$vobj = ArquivoManage::consultarArquivo(3, SearchMounter::MounterByQueryComparerOrder(ArquivoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Arquivo")
								 ->setSubject("Dados Arquivo")
								 ->setDescription("Dados Arquivo")
								 ->setKeywords("Dados Arquivo")
								 ->setCategory("Arquivo");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Arquivo"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Arquivo Disciplina"));
	$objPHPExcel->getActiveSheet()->getStyle("D1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("D")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Arquivo Tipo"));
	$objPHPExcel->getActiveSheet()->getStyle("E1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("E")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("F1", utf8_encode("Usuário"));
	$objPHPExcel->getActiveSheet()->getStyle("F1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("F")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Usuário Responsável"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("I1", utf8_encode("Texto"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("K1", utf8_encode("Data/Hora Edição"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_arquivo"]));
		...
		$ValueArquivoDisciplina = ArquivoDisciplinaManage::buscarArquivoDisciplina(1, $vobj[$i]["id_arquivo_disciplina"]);
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($ValueArquivoDisciplina["titulo"]));
		$ValueArquivo = ArquivoManage::buscarArquivo(1, $vobj[$i]["id_arquivo"]);
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode($ValueArquivo["titulo"]));
		...
		$ValueUsuario = UsuarioManage::buscarUsuario(1, $vobj[$i]["id_usuario"]);
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($ValueUsuario["nome"]));
		$objPHPExcel->getActiveSheet()->setCellValue("H".($i+2), utf8_encode($vobj[$i]["titulo"]));
		$objPHPExcel->getActiveSheet()->setCellValue("I".($i+2), utf8_encode(System::_TextByHtml($vobj[$i]["texto"])));
	...
	$fileName = "Arquivo_".date("d-m-Y_H-i-s").".xlsx";
