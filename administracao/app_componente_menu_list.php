<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("app_componente_menu_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> App Componente Menu: Listagem </td>
  </tr>
  <tr>
  ...
  <tr>
    <td>
	<form id="frm" name="frm" method="get" action="app_componente_menu_list.php">
      <table width="100%" border="0">
        ...
        <tr>
          <td colspan="4" align="right" height="25"><input name="pesquisar" type="submit" class="frm_botao" id="pesquisar" value="Pesquisar" />
            <input name="cancelar" type="button" class="frm_botao" id="cancelar" value="Cancelar" onclick="javascript:window.open('app_componente_menu_list.php', '_parent');" /></td>
        </tr>
		<tr>
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="16%"> Url (Endereço)</td>
        <td height="20" width="16%"> Descrição</td>
        <td height="20" width="16%"> Imagem</td>
        <td height="20" width="16%"> Prioridade</td>
        <td height="20" width="16%"> Tipo</td>
        <td height="20" width="16%"> Status</td>
      ...
      <tr>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i][4])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i][5])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i][6])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=(($vobj[$i][7]==1) ? "<span class='verde'>Ativo</span>" : "<span class='vermelho negrito'>Inativo</span>")?></td>
        <td height="25" bgcolor="#CCCCCC" align="center"><a href="app_componente_menu_priority.php?idatual=<?=$vobj[$i][0]?>&idalterar=<?=$vobj[$i-$h][0]?>" onclick="javascript: return confirm('Deseja realmente Alterar a Prioridade?');"><img src="images/seta1.gif" width="15" height="15" border="0" alt="Alterar Prioridade | Código: <?=$vobj[$i][0]?> por <?=$vobj[$i-$h][0]?>" /></a>&nbsp;<a href="app_componente_menu_view.php?id=<?=$vobj[$i][0]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i][0]?>" /></a>&nbsp;<a href="app_componente_menu_edit.php?id=<?=$vobj[$i][0]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i][0]?>" width="18" height="18" border="0"></a><a href="app_componente_menu_remove.php?id=<?=$vobj[$i][0]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i][0]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
      ?>
	  <tr>
	  	<td colspan="7" style="height:30px; font-weight:bold; font-size:14px; text-align:center">Nenhum registro afetado!</td>
	  </tr>
	  <?
	  	}
	  ?>
