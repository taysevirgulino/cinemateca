<?
	$link_add = "associado_contratacao_add.php";
	$link_edit = "associado_contratacao_edit.php";
	$link_remove = "associado_contratacao_remove.php";
	$link_list = "associado_contratacao_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = AssociadoContratacaoAttribute::ValorBruto();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(AssociadoContratacaoAttribute::ValorBruto(), SearchOrder::Crescente());
		$ordenacao_campo = AssociadoContratacaoAttribute::ValorBruto();
	...
	if(!Validacao::isVazio($exportar)){
		...
		$export = array();
		$export["Object"] = "AssociadoContratacao";
		...
		System::Redirect("associado_contratacao_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = AssociadoContratacaoManage::count( SearchMounter::MounterByQueryCount(AssociadoContratacaoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = AssociadoContratacaoManage::consultar(SearchMounter::MounterByQueryLimit(AssociadoContratacaoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = AssociadoContratacaoAttribute::_GetAllAttributes();
?>