<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("fale_conosco_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Fale Conosco: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="fale_conosco_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="10%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_fale_conosco_assunto&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Assunto">Assunto</a></td>
        <td height="20" width="15%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=nome&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Nome">Nome</a></td>
        <td height="20" width="35%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=mensagem&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Mensagem">Mensagem</a></td>
        <td height="20" width="13%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=datahora&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Data/Hora">Data/Hora</a></td>
        <td height="20" width="10%" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=status&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Status">Lido?</a></td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = FaleConoscoAssuntoManage::buscarFaleConoscoAssunto(1, $vobj[$i]["id_fale_conosco_assunto"]);	print $value["assunto"]; ?></td>
        <td align="left" bgcolor="<?=$bgcolor?>" class="tabela_texto"><?=System::FormatarStringList($vobj[$i]["nome"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i]["mensagem"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarData($vobj[$i]["datahora"], "d/m/y [Hhi]");?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=(($vobj[$i]["status"]==1) ? '<img src="images/icones/s1.gif" title="Sim" class="status" />' : '<img src="images/icones/s0.gif" title="Não" class="status" />')?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_fale_conosco"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="fale_conosco_view.php?id=<?=$vobj[$i]["id_fale_conosco"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_fale_conosco"]?>" /></a>&nbsp;<a href="fale_conosco_edit.php?id=<?=$vobj[$i]["id_fale_conosco"]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_fale_conosco"]?>" width="18" height="18" border="0"></a><a href="fale_conosco_remove.php?id=<?=$vobj[$i]["id_fale_conosco"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_fale_conosco"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
  ...
  <tr>
    <td height="30" align="right">
        ...
        <select name="OperacaoOpcao" id="OperacaoOpcao" class="frm">
            ...
            <option value="status">Inverter Status Leitura</option>
        </select>
