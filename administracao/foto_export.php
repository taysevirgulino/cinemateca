<?
	...
	$vobj = FotoManage::consultarFoto(3, SearchMounter::MounterByQueryComparerOrder(FotoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Foto")
								 ->setSubject("Dados Foto")
								 ->setDescription("Dados Foto")
								 ->setKeywords("Dados Foto")
								 ->setCategory("Foto");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Foto"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Título"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_foto"]));
	...
	$fileName = "Foto_".date("d-m-Y_H-i-s").".xlsx";
