<?
	...
	$vobj = SegmentoManage::consultarSegmento(3, SearchMounter::MounterByQueryComparerOrder(SegmentoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Segmento")
								 ->setSubject("Dados Segmento")
								 ->setDescription("Dados Segmento")
								 ->setKeywords("Dados Segmento")
								 ->setCategory("Segmento");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Segmento"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_segmento"]));
	...
	$fileName = "Segmento_".date("d-m-Y_H-i-s").".xlsx";
