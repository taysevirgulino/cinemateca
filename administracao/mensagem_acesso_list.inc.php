<?
	$link_add = "mensagem_acesso_add.php";
	$link_edit = "mensagem_acesso_edit.php";
	$link_remove = "mensagem_acesso_remove.php";
	$link_list = "mensagem_acesso_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = MensagemAcessoAttribute::Datahora();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(MensagemAcessoAttribute::Datahora(), SearchOrder::Crescente());
		$ordenacao_campo = MensagemAcessoAttribute::Datahora();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("mensagem_acesso_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = MensagemAcessoManage::count( SearchMounter::MounterByQueryCount(MensagemAcessoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = MensagemAcessoManage::consultar(SearchMounter::MounterByQueryLimit(MensagemAcessoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = MensagemAcessoAttribute::_GetAllAttributes();
?>