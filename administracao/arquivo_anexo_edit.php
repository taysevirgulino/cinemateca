<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("arquivo_anexo_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Arquivo Anexo: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdArquivo">&nbsp;Arquivo: * </td>
    ...
    <? } ?>
        ...
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdArquivoHistorico">&nbsp;Arquivo Histórico: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdArquivoHistorico" class="frm" id="FrmIdArquivoHistorico" style="width:500px;">
    <? for($i=0; $i < count($VObjArquivoHistorico); $i++){ ?>
    <option value="<?=$VObjArquivoHistorico[$i]["id_arquivo_historico"]?>" <? if($VObjArquivoHistorico[$i]["id_arquivo_historico"] == $frm_id_arquivo_historico){ print("selected"); }?>><?=$VObjArquivoHistorico[$i]["texto"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="file" name="FrmArquivoFile" id="FrmArquivoFile" class="frm" size="60" maxlength="255" value="">          	<? if(!empty($frm_arquivo)){ ?>
          		<div style="position:relative; float:left; width:100%; margin:5px 0px 0px 0px;">
          			<a href="../files/arquivo_anexo/<?=$frm_arquivo?>" target="_blank" style="text-decoration:underline;"><?=$frm_arquivo?></a>
          			&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp;<label><input type="checkbox" name="FrmArquivoRemove" id="FrmArquivoRemove" value="1" />&nbsp;Remover arquivo?</label>
          		</div>
          	<? } ?>
          </td>
        </tr>
    ...
    <script language="javascript">document.getElementById("FrmIdArquivo").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdArquivoFile':{'l':' Arquivo','t':'labelFrmIdArquivo','r':true},
            'FrmIdArquivoHistoricoFile':{'l':' Arquivo Histórico','t':'labelFrmIdArquivoHistorico','r':true},
            'FrmArquivoFile':{'l':' Arquivo','t':'labelFrmArquivo','r':true},
