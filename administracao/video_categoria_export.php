<?
	...
	$vobj = VideoCategoriaManage::consultarVideoCategoria(3, SearchMounter::MounterByQueryComparerOrder(VideoCategoriaAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados VideoCategoria")
								 ->setSubject("Dados VideoCategoria")
								 ->setDescription("Dados VideoCategoria")
								 ->setKeywords("Dados VideoCategoria")
								 ->setCategory("VideoCategoria");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Vídeo Categoria"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_video_categoria"]));
	...
	$fileName = "VideoCategoria_".date("d-m-Y_H-i-s").".xlsx";
