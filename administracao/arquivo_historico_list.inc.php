<?
	$link_add = "arquivo_historico_add.php";
	$link_edit = "arquivo_historico_edit.php";
	$link_remove = "arquivo_historico_remove.php";
	$link_list = "arquivo_historico_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = ArquivoHistoricoAttribute::Texto();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(ArquivoHistoricoAttribute::Texto(), SearchOrder::Crescente());
		$ordenacao_campo = ArquivoHistoricoAttribute::Texto();
	...
	if(!Validacao::isVazio($exportar)){
		...
		$export = array();
		$export["Object"] = "ArquivoHistorico";
		...
		System::Redirect("arquivo_historico_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = ArquivoHistoricoManage::count( SearchMounter::MounterByQueryCount(ArquivoHistoricoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = ArquivoHistoricoManage::consultar(SearchMounter::MounterByQueryLimit(ArquivoHistoricoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = ArquivoHistoricoAttribute::_GetAllAttributes();
?>