<?
	...
	$vobj = DocumentoManage::consultarDocumento(3, SearchMounter::MounterByQueryComparerOrder(DocumentoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Documento")
								 ->setSubject("Dados Documento")
								 ->setDescription("Dados Documento")
								 ->setKeywords("Dados Documento")
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Documento"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_documento"]));
	...
	$fileName = "Documento_".date("d-m-Y_H-i-s").".xlsx";
