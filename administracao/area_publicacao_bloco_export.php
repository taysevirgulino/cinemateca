<?
	...
	$vobj = AreaPublicacaoBlocoManage::consultarAreaPublicacaoBloco(3, SearchMounter::MounterByQueryComparerOrder(AreaPublicacaoBlocoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AreaPublicacaoBloco")
								 ->setSubject("Dados AreaPublicacaoBloco")
								 ->setDescription("Dados AreaPublicacaoBloco")
								 ->setKeywords("Dados AreaPublicacaoBloco")
								 ->setCategory("AreaPublicacaoBloco");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Área Publicação Bloco"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Legenda"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_area_publicacao_bloco"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($vobj[$i]["legenda"]));
	...
	$fileName = "AreaPublicacaoBloco_".date("d-m-Y_H-i-s").".xlsx";
