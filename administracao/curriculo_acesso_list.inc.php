<?
	$link_add = "curriculo_acesso_add.php";
	$link_edit = "curriculo_acesso_edit.php";
	$link_remove = "curriculo_acesso_remove.php";
	$link_list = "curriculo_acesso_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = CurriculoAcessoAttribute::Datahora();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(CurriculoAcessoAttribute::Datahora(), SearchOrder::Crescente());
		$ordenacao_campo = CurriculoAcessoAttribute::Datahora();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("curriculo_acesso_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = CurriculoAcessoManage::count( SearchMounter::MounterByQueryCount(CurriculoAcessoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = CurriculoAcessoManage::consultar(SearchMounter::MounterByQueryLimit(CurriculoAcessoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = CurriculoAcessoAttribute::_GetAllAttributes();
?>