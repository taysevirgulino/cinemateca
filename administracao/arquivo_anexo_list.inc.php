<?
	$link_add = "arquivo_anexo_add.php";
	$link_edit = "arquivo_anexo_edit.php";
	$link_remove = "arquivo_anexo_remove.php";
	$link_list = "arquivo_anexo_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = ArquivoAnexoAttribute::Arquivo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(ArquivoAnexoAttribute::Arquivo(), SearchOrder::Crescente());
		$ordenacao_campo = ArquivoAnexoAttribute::Arquivo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("arquivo_anexo_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = ArquivoAnexoManage::count( SearchMounter::MounterByQueryCount(ArquivoAnexoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = ArquivoAnexoManage::consultar(SearchMounter::MounterByQueryLimit(ArquivoAnexoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = ArquivoAnexoAttribute::_GetAllAttributes();
?>