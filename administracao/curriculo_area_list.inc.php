<?
	$link_add = "curriculo_area_add.php";
	$link_edit = "curriculo_area_edit.php";
	$link_remove = "curriculo_area_remove.php";
	$link_list = "curriculo_area_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = CurriculoAreaAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(CurriculoAreaAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = CurriculoAreaAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("curriculo_area_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = CurriculoAreaManage::count( SearchMounter::MounterByQueryCount(CurriculoAreaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = CurriculoAreaManage::consultar(SearchMounter::MounterByQueryLimit(CurriculoAreaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = CurriculoAreaAttribute::_GetAllAttributes();
?>