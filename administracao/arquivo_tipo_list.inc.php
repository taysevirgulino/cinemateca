<?
	$link_add = "arquivo_tipo_add.php";
	$link_edit = "arquivo_tipo_edit.php";
	$link_remove = "arquivo_tipo_remove.php";
	$link_list = "arquivo_tipo_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = ArquivoTipoAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(ArquivoTipoAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = ArquivoTipoAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("arquivo_tipo_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = ArquivoTipoManage::count( SearchMounter::MounterByQueryCount(ArquivoTipoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = ArquivoTipoManage::consultar(SearchMounter::MounterByQueryLimit(ArquivoTipoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = ArquivoTipoAttribute::_GetAllAttributes();
?>