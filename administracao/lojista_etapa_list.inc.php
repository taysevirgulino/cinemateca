<?
	$link_add = "lojista_etapa_add.php";
	$link_edit = "lojista_etapa_edit.php";
	$link_remove = "lojista_etapa_remove.php";
	$link_list = "lojista_etapa_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = LojistaEtapaAttribute::Data();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(LojistaEtapaAttribute::Data(), SearchOrder::Crescente());
		$ordenacao_campo = LojistaEtapaAttribute::Data();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("lojista_etapa_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = LojistaEtapaManage::count( SearchMounter::MounterByQueryCount(LojistaEtapaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = LojistaEtapaManage::consultar(SearchMounter::MounterByQueryLimit(LojistaEtapaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = LojistaEtapaAttribute::_GetAllAttributes();
?>