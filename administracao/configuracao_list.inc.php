<?
	$link_add = "configuracao_add.php";
	$link_edit = "configuracao_edit.php";
	$link_remove = "configuracao_remove.php";
	$link_list = "configuracao_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = ConfiguracaoAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(ConfiguracaoAttribute::Nome(), SearchOrder::Crescente());
		$ordenacao_campo = ConfiguracaoAttribute::Nome();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("configuracao_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = SearchMysqlQuery::CountBySql( SearchMounter::MounterByQueryCount(ConfiguracaoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = ConfiguracaoManage::consultarConfiguracao(3, SearchMounter::MounterByQueryLimit(ConfiguracaoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = ConfiguracaoAttribute::_GetAllAttributes();
?>