<?
	$link_add = "fale_conosco_assunto_add.php";
	$link_edit = "fale_conosco_assunto_edit.php";
	$link_remove = "fale_conosco_assunto_remove.php";
	$link_list = "fale_conosco_assunto_list.php";
	...
	//$SearchQueryComparerCollection[] = new SearchQueryComparer(FaleConoscoAssuntoAttribute::IdeOrigem(), SearchComparer::Igual(), SearchCondition::E(), Current::IdeOrigem());
	if(empty($busca_campo)){
		$busca_campo = FaleConoscoAssuntoAttribute::Assunto();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(FaleConoscoAssuntoAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = FaleConoscoAssuntoAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		$export = array();
		$export["Object"] = "FaleConoscoAssunto";
		...
		System::Redirect("fale_conosco_assunto_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = FaleConoscoAssuntoManage::count( SearchMounter::MounterByQueryCount(FaleConoscoAssuntoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = FaleConoscoAssuntoManage::consultar(SearchMounter::MounterByQueryLimit(FaleConoscoAssuntoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = FaleConoscoAssuntoAttribute::_GetAllAttributes();
?>