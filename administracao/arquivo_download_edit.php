<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("arquivo_download_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Arquivo Download: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdArquivoAnexo">&nbsp;Arquivo Anexo: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdArquivoAnexo" class="frm" id="FrmIdArquivoAnexo" style="width:500px;">
    <? for($i=0; $i < count($VObjArquivoAnexo); $i++){ ?>
    <option value="<?=$VObjArquivoAnexo[$i]["id_arquivo_anexo"]?>" <? if($VObjArquivoAnexo[$i]["id_arquivo_anexo"] == $frm_id_arquivo_anexo){ print("selected"); }?>><?=$VObjArquivoAnexo[$i]["arquivo"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
    ...
    <script language="javascript">document.getElementById("FrmIdArquivoAnexo").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdArquivoAnexoFile':{'l':' Arquivo Anexo','t':'labelFrmIdArquivoAnexo','r':true},
            'FrmIdUsuario':{'l':' Usuário','t':'labelFrmIdUsuario','r':true},
            'FrmIp':{'l':' Ip','t':'labelFrmIp','r':true},
