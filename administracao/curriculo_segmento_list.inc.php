<?
	$link_add = "curriculo_segmento_add.php";
	$link_edit = "curriculo_segmento_edit.php";
	$link_remove = "curriculo_segmento_remove.php";
	$link_list = "curriculo_segmento_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = CurriculoSegmentoAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(CurriculoSegmentoAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = CurriculoSegmentoAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		$export = array();
		$export["Object"] = "CurriculoSegmento";
		...
		System::Redirect("curriculo_segmento_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = CurriculoSegmentoManage::count( SearchMounter::MounterByQueryCount(CurriculoSegmentoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = CurriculoSegmentoManage::consultar(SearchMounter::MounterByQueryLimit(CurriculoSegmentoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = CurriculoSegmentoAttribute::_GetAllAttributes();
?>