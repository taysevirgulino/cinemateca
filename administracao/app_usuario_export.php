<?
	...
	$vobj = AppUsuarioManage::consultarAppUsuario(3, SearchMounter::MounterByQueryComparerOrder(AppUsuarioAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AppUsuario")
								 ->setSubject("Dados AppUsuario")
								 ->setDescription("Dados AppUsuario")
								 ->setKeywords("Dados AppUsuario")
								 ->setCategory("AppUsuario");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código App Usuário"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("App Usuário Grupo"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("F1", utf8_encode("Login"));
	$objPHPExcel->getActiveSheet()->getStyle("F1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("F")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Senha"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_app_usuario"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueAppUsuarioGrupo["nome"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("F".($i+2), utf8_encode($vobj[$i]["login"]));
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($vobj[$i]["senha"]));
		$objPHPExcel->getActiveSheet()->setCellValue("H".($i+2), utf8_encode((($vobj[$i]["status"]==1) ? "Ativo" : "Inativo")));
	}

	$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 1);

	$fileName = "AppUsuario_".date("d-m-Y_H-i-s").".xlsx";
