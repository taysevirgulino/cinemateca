<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("noticia_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Notícia: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="noticia_operation.php?back=<?=$link_back;?>">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td width="50%" height="20">Notícia</td>
        <td width="8%" height="20" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=click&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Click">Click</a></td>
        <td width="8%" height="20" align="center"><a href="#" title="Comentários">Com.</a></td>
        <td width="8%" height="20" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=tipo&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Tipo">Tipo</a></td>
        ...
        <td width="13%" height="20" bgcolor="#CCCCCC" align="center">Op&ccedil;&atilde;o</td>
      </tr>
      <?
      	$cont=0;
      ...
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left">
        	<?=System::FormatarStringList($vobj[$i]["chapeu"])?><br />
            <b><?=System::FormatarStringList($vobj[$i]["titulo"])?></b><br />
            <?	$value = EditoriaManage::buscarEditoria(1, $vobj[$i]["id_editoria"]);	print $value["nome"]; ?> / Edição: <?	$value = AppUsuarioManage::buscarAppUsuario(1, $vobj[$i]["id_app_usuario_edicao"]);	print $value["nome"]; ?><br />
            <?=System::FormatarData($vobj[$i]["datahora"], "d/m/y [Hhi]");?>
        </td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=$vobj[$i]["click"]?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=$vobj[$i]["comments"]?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=NoticiaTipo::_Descricao($vobj[$i]["tipo"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=NoticiaStatus::_Descricao($vobj[$i]["status"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_noticia"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center"><a target="_blank" href="<?=Url::Noticia_Slug($vobj[$i]["id_noticia"], $vobj[$i]["slug"])?>"><img src="images/icones/page_world.png" border="0" title="Visualizar no Site" /></a>&nbsp;<a href="noticia_view.php?id=<?=$vobj[$i]["id_noticia"]?>&back=<?=$link_back;?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_noticia"]?>" /></a>&nbsp;<a href="noticia_edit.php?id=<?=$vobj[$i]["id_noticia"]?>&back=<?=$link_back;?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_noticia"]?>" width="18" height="18" border="0"></a><a href="noticia_remove.php?id=<?=$vobj[$i]["id_noticia"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_noticia"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
