<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("lojista_etapa_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Lojista Etapa: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdCronogramaEtapa">&nbsp;Cronograma Etapa: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdCronogramaEtapa" class="frm" id="FrmIdCronogramaEtapa" style="width:500px;">
    <? for($i=0; $i < count($VObjCronogramaEtapa); $i++){ ?>
    <option value="<?=$VObjCronogramaEtapa[$i]["id_cronograma_etapa"]?>" <? if($VObjCronogramaEtapa[$i]["id_cronograma_etapa"] == $frm_id_cronograma_etapa){ print("selected"); }?>><?=$VObjCronogramaEtapa[$i]["titulo"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
    ...
    <? } ?>
        ...
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmData" id="FrmData" class="frm" size="10" maxlength="10" value="<?=$frm_data?>">&nbsp;<input type="button" name="BTData" id="BTData" class="frm" value="..." ><script type="text/javascript"> Calendar.setup({ inputField:"FrmData", ifFormat:"%d/%m/%Y", showsTime:true, button:"BTData", singleClick:true, step:1 }); </script>&nbsp;<span class="frm_exemplo">Ex: <?=date("d/m/Y")?></span>
          </td>
        </tr>
        <tr>
    ...
    <script language="javascript">document.getElementById("FrmIdCronogramaEtapa").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdCronogramaEtapa':{'l':' Cronograma Etapa','t':'labelFrmIdCronogramaEtapa','r':true},
            'FrmIdLojista':{'l':' Lojista','t':'labelFrmIdLojista','r':true},
            'FrmIdUsuario':{'l':' Usuário','t':'labelFrmIdUsuario','r':true},
            'FrmData':{'l':' Data','t':'labelFrmData','f':'date'},
