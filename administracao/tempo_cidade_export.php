<?
	...
	$vobj = TempoCidadeManage::consultarTempoCidade(3, SearchMounter::MounterByQueryComparerOrder(TempoCidadeAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados TempoCidade")
								 ->setSubject("Dados TempoCidade")
								 ->setDescription("Dados TempoCidade")
								 ->setKeywords("Dados TempoCidade")
								 ->setCategory("TempoCidade");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Tempo Cidade"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Uf"));
	$objPHPExcel->getActiveSheet()->getStyle("D1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("D")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Código"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_tempo_cidade"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($vobj[$i]["uf"]));
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode($vobj[$i]["codigo"]));
	...
	$fileName = "TempoCidade_".date("d-m-Y_H-i-s").".xlsx";
