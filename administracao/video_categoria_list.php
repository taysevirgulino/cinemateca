<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("video_categoria_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Vídeo Categoria: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="video_categoria_operation.php?back=<?=$link_back;?>">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      ...
      <tr>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_video_categoria"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center"><a href="video_categoria_priority.php?idatual=<?=$vobj[$i]["id_video_categoria"]?>&idalterar=<?=$vobj[$i-$h]["id_video_categoria"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente Alterar a Prioridade?');"><img src="images/seta1.gif" width="15" height="15" border="0" alt="Alterar Prioridade | Código: <?=$vobj[$i]["id_video_categoria"]?> por <?=$vobj[$i-$h]["id_video_categoria"]?>" /></a>&nbsp;<a href="video_categoria_view.php?id=<?=$vobj[$i]["id_video_categoria"]?>&back=<?=$link_back;?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_video_categoria"]?>" /></a>&nbsp;<a href="video_categoria_edit.php?id=<?=$vobj[$i]["id_video_categoria"]?>&back=<?=$link_back;?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_video_categoria"]?>" width="18" height="18" border="0"></a><a href="video_categoria_remove.php?id=<?=$vobj[$i]["id_video_categoria"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_video_categoria"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
