<?
	$link_add = "arquivo_download_add.php";
	$link_edit = "arquivo_download_edit.php";
	$link_remove = "arquivo_download_remove.php";
	$link_list = "arquivo_download_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = ArquivoDownloadAttribute::Datahora();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(ArquivoDownloadAttribute::Datahora(), SearchOrder::Crescente());
		$ordenacao_campo = ArquivoDownloadAttribute::Datahora();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("arquivo_download_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = ArquivoDownloadManage::count( SearchMounter::MounterByQueryCount(ArquivoDownloadAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = ArquivoDownloadManage::consultar(SearchMounter::MounterByQueryLimit(ArquivoDownloadAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = ArquivoDownloadAttribute::_GetAllAttributes();
?>