<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("galeria_album_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Galeria Álbum: Listagem </td>
  </tr>
  <tr>
      ...
      <table width="100%" border="0">
        ...
        <tr>
          <td colspan="4" align="right" height="25"><input name="pesquisar" type="submit" class="frm_botao" id="pesquisar" value="Pesquisar" />
            <input name="cancelar" type="button" class="frm_botao" id="cancelar" value="Cancelar" onclick="javascript:window.open('galeria_album_list.php', '_parent');" />
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="galeria_album_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="20%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_galeria_categoria&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Galeria Categoria">Categoria</a></td>
        <td height="20" width="25%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=nome&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Nome">Nome</a></td>
        <td height="20" width="10%" align="center">Fotos</td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = GaleriaCategoriaManage::buscarGaleriaCategoria(1, $vobj[$i]["id_galeria_categoria"]);	print $value["nome"]; ?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i]["nome"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=GaleriaFotoManage2::QuantidadeByAlbum($vobj[$i]["id_galeria_album"])?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_galeria_album"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="<?=Url::GaleriaAlbum($vobj[$i]["id_galeria_album"], $vobj[$i]["identificador"], $vobj[$i]["nome"])?>" target="_blank"><img src="images/icones/world_link.gif" border="0" title="Visualizar no Site" /></a>&nbsp;<a href="galeria_foto_list.php?ida=<?=$vobj[$i]["id_galeria_album"]?>"><img src="menu/icones/imageattach.gif" width="18" height="18" border="0" alt="Fotos | Código: <?=$vobj[$i]["id_galeria_album"]?>" /></a>&nbsp;<a href="galeria_album_view.php?id=<?=$vobj[$i]["id_galeria_album"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_galeria_album"]?>" /></a>&nbsp;<a href="galeria_album_edit.php?id=<?=$vobj[$i]["id_galeria_album"]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_galeria_album"]?>" width="18" height="18" border="0"></a><a href="galeria_album_remove.php?id=<?=$vobj[$i]["id_galeria_album"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_galeria_album"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
