<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("emkt_lista_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Newsletter Lista: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="emkt_lista_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="70%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=nome&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Nome">Nome</a></td>
        <td height="20" width="10%" align="center">Contatos</td>
      ...
      <tr>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=EmktContatoManage::ContatosCountByLista($vobj[$i]["id_emkt_lista"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_emkt_lista"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="emkt_lista_view.php?id=<?=$vobj[$i]["id_emkt_lista"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_emkt_lista"]?>" /></a>&nbsp;<a href="emkt_lista_edit.php?id=<?=$vobj[$i]["id_emkt_lista"]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_emkt_lista"]?>" width="18" height="18" border="0"></a><a href="emkt_lista_remove.php?id=<?=$vobj[$i]["id_emkt_lista"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_emkt_lista"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
