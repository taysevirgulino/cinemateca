			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						PaginaManage::excluirPagina($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new Pagina();
						if(!$obj_atual->buscarPagina(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new Pagina();
						if(!$obj_alterar->buscarPagina(1, $itens[1])){ System::Redirect($link_list); }
