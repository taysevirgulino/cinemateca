<?
	...
	$vobj = MensagemRespostaManage::consultarMensagemResposta(3, SearchMounter::MounterByQueryComparerOrder(MensagemRespostaAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados MensagemResposta")
								 ->setSubject("Dados MensagemResposta")
								 ->setDescription("Dados MensagemResposta")
								 ->setKeywords("Dados MensagemResposta")
								 ->setCategory("MensagemResposta");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Mensagem Resposta"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Mensagem"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Texto"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("H1", utf8_encode("Ip"));
	$objPHPExcel->getActiveSheet()->getStyle("H1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("H")->setAutoSize(true);


	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_mensagem_resposta"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueMensagem = MensagemManage::buscarMensagem(1, $vobj[$i]["id_mensagem"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueMensagem["titulo"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode(System::_TextByHtml($vobj[$i]["texto"])));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("H".($i+2), utf8_encode($vobj[$i]["ip"]));
	}

	$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 1);
	$objPHPExcel->getActiveSheet()->setAutoFilter( $objPHPExcel->getActiveSheet()->calculateWorksheetDimension() );

	$fileName = "MensagemResposta_".date("d-m-Y_H-i-s").".xlsx";
