<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("emkt_disparo_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Emkt Disparo: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
      ...
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdEmkt" class="frm" id="FrmIdEmkt" style="width:500px;">
    <? for($i=0; $i < count($VObjEmkt); $i++){ ?>
    <option value="<?=$VObjEmkt[$i][0]?>" <? if($VObjEmkt[$i][0] == $frm_id_emkt){ print("selected"); }?>><?=$VObjEmkt[$i]["titulo"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmAgendamento">&nbsp;Agendamento:  </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmAgendamento" id="FrmAgendamento" class="frm" size="19" maxlength="19" value="<?=$frm_agendamento?>" onkeydown="fncFormataData('frm', 'FrmAgendamento')">&nbsp;<input type="button" name="BTAgendamento" id="BTAgendamento" class="frm" value="..." ><script type="text/javascript"> Calendar.setup({ inputField:"FrmAgendamento", ifFormat:"%d/%m/%Y %H:%M:%S", showsTime:true, button:"BTAgendamento", singleClick:true, step:1 }); </script>&nbsp;<span class="frm_exemplo">Ex: <?=date("d/m/Y H:i:s")?></span>
          </td>
        </tr>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmResultado">&nbsp;Resultado:  </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<?php $CKEditor = new CKEditor(); $CKEditor->returnOutput = true; $CKEditor->basePath="script/editorhtml/"; $CKEditor->config['height'] = 500; print $CKEditor->editor("FrmResultado", $frm_resultado); ?>
          </td>
        </tr>
        <tr>
    ...
    <script language="javascript">document.getElementById("FrmIdEmkt").focus();</script>
	<script language="javascript">
    <!--
		function _Validar(){
			var objFrmResultado = CKEDITOR.instances.FrmResultado;
			if(objFrmResultado != null){
				objFrmResultado.updateElement();
			}
			return vFrm.exec();
		}
        var a_fields = {
            'FrmIdEmkt':{'l':' Emkt','t':'labelFrmIdEmkt','r':true},
            'FrmAgendamento':{'l':' Agendamento','t':'labelFrmAgendamento','f':'date'},
            'FrmResultado':{'l':' Resultado','t':'labelFrmResultado'},
