<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("acesso_rapido_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Acesso Rápido: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="acesso_rapido_operation.php?back=<?=$link_back;?>">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td width="30%" height="20" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=nome&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Nome">Nome</a></td>
        <td width="30%" height="20" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=url&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Url (Endereço)">Url (Endereço)</a></td>
        <td width="10%" height="20" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=target&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Abrir">Abrir</a></td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=AcessoRapidoManage2::HerancaByString($vobj[$i]["id_acesso_rapido"])?> (<?	$value = AcessoRapidoBlocoManage::buscarAcessoRapidoBloco(1, $vobj[$i]["id_acesso_rapido_bloco"]);	print $value["nome"]; ?>)</td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i]["url"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=(($vobj[$i]["target"]=='_blank') ? "Nova Página" : "Mesma Página")?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_acesso_rapido"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center"><a href="acesso_rapido_priority.php?idatual=<?=$vobj[$i]["id_acesso_rapido"]?>&idalterar=<?=$vobj[$i-$h]["id_acesso_rapido"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente Alterar a Prioridade?');"><img src="images/seta1.gif" width="15" height="15" border="0" title="Alterar Prioridade | Código: <?=$vobj[$i]["id_acesso_rapido"]?> por <?=$vobj[$i-$h]["id_acesso_rapido"]?>" /></a>&nbsp;<a href="acesso_rapido_view.php?id=<?=$vobj[$i]["id_acesso_rapido"]?>&back=<?=$link_back;?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_acesso_rapido"]?>" /></a>&nbsp;<a href="acesso_rapido_edit.php?id=<?=$vobj[$i]["id_acesso_rapido"]?>&back=<?=$link_back;?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_acesso_rapido"]?>" width="18" height="18" border="0"></a><a href="acesso_rapido_remove.php?id=<?=$vobj[$i]["id_acesso_rapido"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_acesso_rapido"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
