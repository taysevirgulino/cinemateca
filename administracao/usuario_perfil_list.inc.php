<?
	$link_add = "usuario_perfil_add.php";
	$link_edit = "usuario_perfil_edit.php";
	$link_remove = "usuario_perfil_remove.php";
	$link_list = "usuario_perfil_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = UsuarioPerfilAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(UsuarioPerfilAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = UsuarioPerfilAttribute::Titulo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("usuario_perfil_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = UsuarioPerfilManage::count( SearchMounter::MounterByQueryCount(UsuarioPerfilAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = UsuarioPerfilManage::consultar(SearchMounter::MounterByQueryLimit(UsuarioPerfilAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = UsuarioPerfilAttribute::_GetAllAttributes();
?>