<?
	...
	$vobj = AcessoRapidoManage::consultarAcessoRapido(3, SearchMounter::MounterByQueryComparerOrder(AcessoRapidoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AcessoRapido")
								 ->setSubject("Dados AcessoRapido")
								 ->setDescription("Dados AcessoRapido")
								 ->setKeywords("Dados AcessoRapido")
								 ->setCategory("AcessoRapido");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Acesso Rápido"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Acesso Rápido Pai"));
	$objPHPExcel->getActiveSheet()->getStyle("C1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("C")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Acesso Rápido Bloco"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("F1", utf8_encode("Url (Endereço)"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_acesso_rapido"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueAcessoRapido = AcessoRapidoManage::buscarAcessoRapido(1, $vobj[$i]["id_acesso_rapido"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueAcessoRapido["nome"]));
		$ValueAcessoRapidoBloco = AcessoRapidoBlocoManage::buscarAcessoRapidoBloco(1, $vobj[$i]["id_acesso_rapido_bloco"]);
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($ValueAcessoRapidoBloco["nome"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode((($vobj[$i]["target"]=='_blank') ? "Nova Página" : "Mesma Página")));
	...
	$fileName = "AcessoRapido_".date("d-m-Y_H-i-s").".xlsx";
