<?
	...
	$vobj = VideoManage::consultarVideo(3, SearchMounter::MounterByQueryComparerOrder(VideoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Video")
								 ->setSubject("Dados Video")
								 ->setDescription("Dados Video")
								 ->setKeywords("Dados Video")
								 ->setCategory("Video");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Vídeo"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Vídeo Categoria"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_video"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueVideoCategoria = VideoCategoriaManage::buscarVideoCategoria(1, $vobj[$i]["id_video_categoria"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueVideoCategoria["nome"]));
	...
	$fileName = "Video_".date("d-m-Y_H-i-s").".xlsx";
