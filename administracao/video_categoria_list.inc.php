<?
	$link_add = "video_categoria_add.php";
	$link_edit = "video_categoria_edit.php";
	$link_remove = "video_categoria_remove.php";
	$link_list = "video_categoria_list.php";
	...
	$SearchQueryComparerCollection[] = new SearchQueryComparer(VideoCategoriaAttribute::IdeOrigem(), SearchComparer::Igual(), SearchCondition::E(), Current::IdeOrigem());
	if(empty($busca_campo)){
		$busca_campo = VideoCategoriaAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(VideoCategoriaAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = VideoCategoriaAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("video_categoria_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = VideoCategoriaManage::count( SearchMounter::MounterByQueryCount(VideoCategoriaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = VideoCategoriaManage::consultar(SearchMounter::MounterByQueryLimit(VideoCategoriaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = VideoCategoriaAttribute::_GetAllAttributes();
?>