<?
	$link_add = "eixo_categoria_add.php";
	$link_edit = "eixo_categoria_edit.php";
	$link_remove = "eixo_categoria_remove.php";
	$link_list = "eixo_categoria_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = EixoCategoriaAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(EixoCategoriaAttribute::Nome(), SearchOrder::Crescente());
		$ordenacao_campo = EixoCategoriaAttribute::Nome();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("eixo_categoria_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = EixoCategoriaManage::count( SearchMounter::MounterByQueryCount(EixoCategoriaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = EixoCategoriaManage::consultar(SearchMounter::MounterByQueryLimit(EixoCategoriaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = EixoCategoriaAttribute::_GetAllAttributes();
?>