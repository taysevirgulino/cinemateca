<?
	$link_add = "area_publicacao_add.php";
	$link_edit = "area_publicacao_edit.php";
	$link_remove = "area_publicacao_remove.php";
	$link_list = "area_publicacao_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = AreaPublicacaoAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(AreaPublicacaoAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = AreaPublicacaoAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("area_publicacao_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = AreaPublicacaoManage::count( SearchMounter::MounterByQueryCount(AreaPublicacaoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$paginacao = new Paginacao(300, $vobjCount, $PG, $link_back);
	...
	$vobj = AreaPublicacaoManage::consultar(SearchMounter::MounterByQueryLimit(AreaPublicacaoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = AreaPublicacaoAttribute::_GetAllAttributes();
?>