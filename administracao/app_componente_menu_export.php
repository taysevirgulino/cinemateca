<?
	...
	$vobj = AppComponenteMenuManage::consultarAppComponenteMenu(3, SearchMounter::MounterByQueryComparerOrder(AppComponenteMenuAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AppComponenteMenu")
								 ->setSubject("Dados AppComponenteMenu")
								 ->setDescription("Dados AppComponenteMenu")
								 ->setKeywords("Dados AppComponenteMenu")
								 ->setCategory("AppComponenteMenu");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código App Componente Menu"));
	$objPHPExcel->getActiveSheet()->getStyle("A1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("A")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("B1", utf8_encode("App Componente Menu Pai"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Imagem"));
	$objPHPExcel->getActiveSheet()->getStyle("E1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("E")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("F1", utf8_encode("Prioridade"));
	$objPHPExcel->getActiveSheet()->getStyle("F1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("F")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Tipo"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_app_componente_menu"]));
		$ValueAppComponenteMenu = AppComponenteMenuManage::buscarAppComponenteMenu(1, $vobj[$i]["id_app_componente_menu"]);
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($ValueAppComponenteMenu["url"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode($vobj[$i]["imagem"]));
		$objPHPExcel->getActiveSheet()->setCellValue("F".($i+2), utf8_encode($vobj[$i]["prioridade"]));
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode($vobj[$i]["tipo"]));
		$objPHPExcel->getActiveSheet()->setCellValue("H".($i+2), utf8_encode((($vobj[$i]["status"]==1) ? "Ativo" : "Inativo")));
	}

	$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 1);

	$fileName = "AppComponenteMenu_".date("d-m-Y_H-i-s").".xlsx";
