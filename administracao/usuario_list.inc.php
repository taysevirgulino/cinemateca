<?
	$link_add = "usuario_add.php";
	$link_edit = "usuario_edit.php";
	$link_remove = "usuario_remove.php";
	$link_list = "usuario_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = UsuarioAttribute::Nome();
	}
	
	//$SearchQueryComparerCollection[] = new SearchQueryComparer(UsuarioAttribute::IdUsuarioPerfil(), SearchComparer::Diferente(), SearchCondition::E(), 1);
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(UsuarioAttribute::Nome(), SearchOrder::Crescente());
		$ordenacao_campo = UsuarioAttribute::Nome();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("usuario_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = UsuarioManage::count( SearchMounter::MounterByQueryCount(UsuarioAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = UsuarioManage::consultar(SearchMounter::MounterByQueryLimit(UsuarioAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = UsuarioAttribute::_GetAllAttributes();
?>