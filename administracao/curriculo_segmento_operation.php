			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						CurriculoSegmentoManage::excluirCurriculoSegmento($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new CurriculoSegmento();
						if(!$obj_atual->buscarCurriculoSegmento(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new CurriculoSegmento();
						if(!$obj_alterar->buscarCurriculoSegmento(1, $itens[1])){ System::Redirect($link_list); }
				...
				case "status" : {
					for ($i=0; $i < count($itens); $i++){
						...
						if($obj->buscarCurriculoSegmento(1, $itens[$i])){
