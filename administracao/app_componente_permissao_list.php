<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("app_componente_permissao_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> App Componente Permissão: Listagem </td>
  </tr>
  <tr>
  ...
  <tr>
    <td>
	<form id="frm" name="frm" method="get" action="app_componente_permissao_list.php">
      <table width="100%" border="0">
        ...
        <tr>
          <td colspan="4" align="right" height="25"><input name="pesquisar" type="submit" class="frm_botao" id="pesquisar" value="Pesquisar" />
            <input name="cancelar" type="button" class="frm_botao" id="cancelar" value="Cancelar" onclick="javascript:window.open('app_componente_permissao_list.php', '_parent');" /></td>
        </tr>
		<tr>
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="40%"> App Componente</td>
        <td height="20" width="40%"> Permissão</td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = AppComponenteManage::buscarAppComponente(1, $vobj[$i][2]);	print $value["data"]; ?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i][3])?></td>
        <td height="25" bgcolor="#CCCCCC" align="center"><a href="app_componente_permissao_view.php?id=<?=$vobj[$i][0]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i][0]?>" /></a>&nbsp;<a href="app_componente_permissao_edit.php?id=<?=$vobj[$i][0]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i][0]?>" width="18" height="18" border="0"></a><a href="app_componente_permissao_remove.php?id=<?=$vobj[$i][0]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i][0]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
