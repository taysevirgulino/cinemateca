<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("arquivo_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Arquivo: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <? } ?>
        ...
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdArquivoDisciplina">&nbsp;Arquivo Disciplina: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdArquivoDisciplina" class="frm" id="FrmIdArquivoDisciplina" style="width:500px;">
    <? for($i=0; $i < count($VObjArquivoDisciplina); $i++){ ?>
    <option value="<?=$VObjArquivoDisciplina[$i]["id_arquivo_disciplina"]?>" <? if($VObjArquivoDisciplina[$i]["id_arquivo_disciplina"] == $frm_id_arquivo_disciplina){ print("selected"); }?>><?=$VObjArquivoDisciplina[$i]["titulo"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdArquivoTipo">&nbsp;Arquivo: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdArquivo" class="frm" id="FrmIdArquivo" style="width:500px;">
    <? for($i=0; $i < count($VObjArquivo); $i++){ ?>
    <option value="<?=$VObjArquivo[$i]["id_arquivo"]?>" <? if($VObjArquivo[$i]["id_arquivo"] == $frm_id_arquivo){ print("selected"); }?>><?=$VObjArquivo[$i]["titulo"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
    ...
    <? } ?>
        ...
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdUsuarioResponsavel">&nbsp;Usuário: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdUsuario" class="frm" id="FrmIdUsuario" style="width:500px;">
    <? for($i=0; $i < count($VObjUsuario); $i++){ ?>
    <option value="<?=$VObjUsuario[$i]["id_usuario"]?>" <? if($VObjUsuario[$i]["id_usuario"] == $frm_id_usuario){ print("selected"); }?>><?=$VObjUsuario[$i]["nome"]?></option>
    ...
    <!--
        ...
        var a_fields = {
            ...
            'FrmIdArquivoDisciplinaFile':{'l':' Arquivo Disciplina','t':'labelFrmIdArquivoDisciplina','r':true},
            'FrmIdArquivoTipoFile':{'l':' Arquivo','t':'labelFrmIdArquivoTipo','r':true},
            ...
            'FrmDatahoraEdicao':{'l':' Data/Hora Edição','t':'labelFrmDatahoraEdicao','r':true,'f':'date'},
