			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						AudioCategoriaManage::excluirAudioCategoria($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new AudioCategoria();
						if(!$obj_atual->buscarAudioCategoria(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new AudioCategoria();
						if(!$obj_alterar->buscarAudioCategoria(1, $itens[1])){ System::Redirect($link_list); }
