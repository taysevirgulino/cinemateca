<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("enquete_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Enquete: Listagem </td>
  </tr>
  <tr>
      ...
      <table width="100%" border="0">
        ...
        <tr>
          <td colspan="4" align="right" height="25">
            ...
            <input name="cancelar" type="button" class="frm_botao" id="cancelar" value="Cancelar" onclick="javascript:window.open('enquete_list.php', '_parent');" />
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="enquete_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="40%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=pergunta&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Pergunta">Pergunta</a></td>
        <td height="20" width="20%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=data_inicial&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Data Inicial">Data</a></td>
        <td height="20" width="10%" align="center">Votos</td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarStringList($vobj[$i]["pergunta"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?=System::FormatarData($vobj[$i]["data_inicial"], "d/m/y");?> à <?=System::FormatarData($vobj[$i]["data_final"], "d/m/y");?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=EnqueteRespostaManage2::QuantidadeByEnquete($vobj[$i]["id_enquete"])?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_enquete"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="<?=Url::Enquete($vobj[$i]["id_enquete"], $vobj[$i]["identificador"], $articulista." ".$vobj[$i]["pergunta"])?>" target="_blank"><img src="images/icones/world_link.gif" border="0" alt="Visualizar no Site" /></a>&nbsp;<a href="enquete_view.php?id=<?=$vobj[$i]["id_enquete"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" alt="Visualizar | Código: <?=$vobj[$i]["id_enquete"]?>" /></a>&nbsp;<a href="enquete_edit.php?id=<?=$vobj[$i]["id_enquete"]?>"><img src="images/botao/Edit.gif" alt="Editar | Código: <?=$vobj[$i]["id_enquete"]?>" width="18" height="18" border="0"></a><a href="enquete_remove.php?id=<?=$vobj[$i]["id_enquete"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" alt="Excluir | Código: <?=$vobj[$i]["id_enquete"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
