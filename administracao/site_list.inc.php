<?
	...
	if(empty($busca_campo)){
		$busca_campo = SiteAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(SiteAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = SiteAttribute::Titulo();
		$ordenacao_ordem = SearchOrder::Crescente();
	}
	$ordenacao_ordem_inverso = SearchOrder::_Inverso($ordenacao_ordem);

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = SiteManage::count( SearchMounter::MounterByQueryCount(SiteAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = SiteManage::consultar(SearchMounter::MounterByQueryLimit(SiteAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = SiteAttribute::_GetAllAttributes();
?>