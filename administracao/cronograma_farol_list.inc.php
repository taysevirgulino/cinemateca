<?
	$link_add = "cronograma_farol_add.php";
	$link_edit = "cronograma_farol_edit.php";
	$link_remove = "cronograma_farol_remove.php";
	$link_list = "cronograma_farol_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = CronogramaFarolAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(CronogramaFarolAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = CronogramaFarolAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("cronograma_farol_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = CronogramaFarolManage::count( SearchMounter::MounterByQueryCount(CronogramaFarolAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = CronogramaFarolManage::consultar(SearchMounter::MounterByQueryLimit(CronogramaFarolAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = CronogramaFarolAttribute::_GetAllAttributes();
?>