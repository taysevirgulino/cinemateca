<?
	...
	if(empty($busca_campo)){
		$busca_campo = JornalEdicaoAttribute::Numero();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(JornalEdicaoAttribute::DataInicial(), SearchOrder::Decrescente());
		$ordenacao_campo = JornalEdicaoAttribute::DataInicial();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("jornal_edicao_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = JornalEdicaoManage::count( SearchMounter::MounterByQueryCount(JornalEdicaoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = JornalEdicaoManage::consultar(SearchMounter::MounterByQueryLimit(JornalEdicaoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = JornalEdicaoAttribute::_GetAllAttributes();
?>