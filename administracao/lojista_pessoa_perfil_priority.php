<?
	...
	$link_list = ((Validacao::isVazio(System::_REQUEST("back"))) ? "lojista_pessoa_perfil_list.php" : base64_decode(System::_GET("back")) );
	...
	if(!$obj_atual->buscarAttribute(LojistaPessoaPerfilAttribute::IdLojistaPessoaPerfil(), $ID_ATUAL)){ System::Redirect($link_list); }

	$obj_alterar = new LojistaPessoaPerfil();
	if(!$obj_alterar->buscarAttribute(LojistaPessoaPerfilAttribute::IdLojistaPessoaPerfil(), $ID_ALTERAR)){ System::Redirect($link_list); }

	$aux = $obj_atual->getPrioridade();
	$obj_atual->setPrioridade( $obj_alterar->getPrioridade() );
	$obj_alterar->setPrioridade( $aux );

	$obj_atual->alterarAtributo(LojistaPessoaPerfilAttribute::Prioridade());
	$obj_alterar->alterarAtributo(LojistaPessoaPerfilAttribute::Prioridade());
