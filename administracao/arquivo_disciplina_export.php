<?
	...
	$vobj = ArquivoDisciplinaManage::consultarArquivoDisciplina(3, SearchMounter::MounterByQueryComparerOrder(ArquivoDisciplinaAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados ArquivoDisciplina")
								 ->setSubject("Dados ArquivoDisciplina")
								 ->setDescription("Dados ArquivoDisciplina")
								 ->setKeywords("Dados ArquivoDisciplina")
								 ->setCategory("ArquivoDisciplina");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Arquivo Disciplina"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_arquivo_disciplina"]));
	...
	$fileName = "ArquivoDisciplina_".date("d-m-Y_H-i-s").".xlsx";
