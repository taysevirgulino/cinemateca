<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("app_componente_permissao_menu.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Permiss&atilde;o: Menu </td>
  </tr>
  <tr>
  ...
  <tr>
    <td>
	<form action="app_componente_permissao_menu.php?id=<?=$ID?>" method="post" name="frm" id="frm" enctype="multipart/form-data">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        ...
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto">&nbsp; Menus: </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          ...
          <?
				$ct = AppComponenteMenuManage::consultarAppComponenteMenu(2, "ORDER BY descricao");
				$new = array();
				$ac = new AppComponenteMenu();

				for($j=0; $j < count($ct); $j++){
					$new[$j][0] = $ac->montarFiliacao(intval( $ct[$j]["id_app_componente_menu"] ), "");
					$new[$j][1] = $ct[$j]["id_app_componente_menu"];
				}
				sort($new);
				...
				for($j=0; $j < count($new); $j++){
					...
					print('<input type="checkbox" onmouseover="Mudar(this);" id="rb'.$cont.'" name="rb'.$cont.'" value="'.$new[$j][1].'" '.$checked.' /><span  style="font-weight:normal;">'.$new[$j][0].'</span><br />');
					$cont++;
				}
