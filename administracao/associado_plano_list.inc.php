<?
	$link_add = "associado_plano_add.php";
	$link_edit = "associado_plano_edit.php";
	$link_remove = "associado_plano_remove.php";
	$link_list = "associado_plano_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = AssociadoPlanoAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(AssociadoPlanoAttribute::Prioridade(), SearchOrder::Crescente());
		$ordenacao_campo = AssociadoPlanoAttribute::Prioridade();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("associado_plano_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = AssociadoPlanoManage::count( SearchMounter::MounterByQueryCount(AssociadoPlanoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = AssociadoPlanoManage::consultar(SearchMounter::MounterByQueryLimit(AssociadoPlanoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = AssociadoPlanoAttribute::_GetAllAttributes();
?>