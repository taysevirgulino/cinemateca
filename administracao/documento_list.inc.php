<?
	$link_add = "documento_add.php";
	$link_edit = "documento_edit.php";
	$link_remove = "documento_remove.php";
	$link_list = "documento_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = DocumentoAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(DocumentoAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = DocumentoAttribute::Titulo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("documento_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = DocumentoManage::count( SearchMounter::MounterByQueryCount(DocumentoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = DocumentoManage::consultar(SearchMounter::MounterByQueryLimit(DocumentoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = DocumentoAttribute::_GetAllAttributes();
?>