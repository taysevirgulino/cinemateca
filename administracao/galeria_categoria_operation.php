			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						GaleriaCategoriaManage::excluirGaleriaCategoria($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new GaleriaCategoria();
						if(!$obj_atual->buscarGaleriaCategoria(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new GaleriaCategoria();
						if(!$obj_alterar->buscarGaleriaCategoria(1, $itens[1])){ System::Redirect($link_list); }
				...
				case "status" : {
					for ($i=0; $i < count($itens); $i++){
						...
						if($obj->buscarGaleriaCategoria(1, $itens[$i])){
