<?
	$link_add = "noticia_add.php";
	$link_edit = "noticia_edit.php";
	$link_remove = "noticia_remove.php";
	$link_list = "noticia_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = NoticiaAttribute::Titulo();
	}
	
	//$SearchQueryComparerCollection[] = new SearchQueryComparer(NoticiaAttribute::IdeOrigem(), SearchComparer::Igual(), SearchCondition::E(), Current::IdeOrigem());
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(NoticiaAttribute::Datahora(), SearchOrder::Decrescente());
		$ordenacao_campo = NoticiaAttribute::Datahora();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("noticia_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
//$vobjCount = SearchMysqlQuery::CountBySql( SearchMounter::MounterByQueryCount(NoticiaAttribute::_Table(), $SearchQueryComparerCollection) );
	$vobjCount = NoticiaManage::count( SearchMounter::MounterByQueryCount(NoticiaAttribute::_Table(), $SearchQueryComparerCollection) );
...
//$vobj = NoticiaManage::consultarNoticia(3, SearchMounter::MounterByQueryLimit(NoticiaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));
	$vobj = NoticiaManage::consultar( SearchMounter::MounterByQueryLimit(NoticiaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()) );

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = NoticiaAttribute::_GetAllAttributes();
?>