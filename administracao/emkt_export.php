<?
	...
	$vobj = EmktManage::consultarEmkt(3, SearchMounter::MounterByQueryComparerOrder(EmktAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Emkt")
								 ->setSubject("Dados Emkt")
								 ->setDescription("Dados Emkt")
								 ->setKeywords("Dados Emkt")
								 ->setCategory("Emkt");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Emkt"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Assunto"));
	$objPHPExcel->getActiveSheet()->getStyle("D1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("D")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Remetente Nome"));
	$objPHPExcel->getActiveSheet()->getStyle("E1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("E")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("F1", utf8_encode("Remetente E-mail"));
	$objPHPExcel->getActiveSheet()->getStyle("F1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("F")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("G1", utf8_encode("Mensagem Html"));
	$objPHPExcel->getActiveSheet()->getStyle("G1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("G")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("H1", utf8_encode("Mensagem Texto"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_emkt"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($vobj[$i]["assunto"]));
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode($vobj[$i]["remetente_nome"]));
		$objPHPExcel->getActiveSheet()->setCellValue("F".($i+2), utf8_encode($vobj[$i]["remetente_email"]));
		$objPHPExcel->getActiveSheet()->setCellValue("G".($i+2), utf8_encode(System::_TextByHtml($vobj[$i]["mensagem_html"])));
		$objPHPExcel->getActiveSheet()->setCellValue("H".($i+2), utf8_encode(System::_TextByHtml($vobj[$i]["mensagem_texto"])));
		$objPHPExcel->getActiveSheet()->setCellValue("I".($i+2), utf8_encode(System::FormatarData($vobj[$i]["datahora"], "d/m/Y H:i:s")));
	}

	$objPHPExcel->getActiveSheet()->getPageSetup()->setRowsToRepeatAtTopByStartAndEnd(1, 1);

	$fileName = "Emkt_".date("d-m-Y_H-i-s").".xlsx";
