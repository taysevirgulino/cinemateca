<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("banner_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Banner: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="banner_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="15%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_banner_local&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Banner Local">Local</a></td>
        <td height="20" width="20%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=descricao&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Descrição">Descrição</a></td>
        <td height="20" width="8%" align="left">WxH</td>
        <td height="20" width="10%" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=periodo_status&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Período Status">Período</a></td>
        <td height="20" width="10%" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=tipo&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Tipo">Tipo</a></td>
        <td height="20" width="10%" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=status&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Status">Status</a></td>
        ...
        <td width="12%" height="20" bgcolor="#CCCCCC" align="center">Op&ccedil;&atilde;o</td>
      </tr>
      <?
      	$cont=0;
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = BannerLocalManage::buscarBannerLocal(1, $vobj[$i]["id_banner_local"]);	print $value["nome"]; ?></td>
        <td align="left" bgcolor="<?=$bgcolor?>" class="tabela_texto"><?=System::FormatarStringList($vobj[$i]["descricao"])?></td>
        <td align="left" bgcolor="<?=$bgcolor?>" class="tabela_texto"><?=System::FormatarStringList($vobj[$i]["width"])?> x <?=System::FormatarStringList($vobj[$i]["height"])?></td>
        <td align="center" bgcolor="<?=$bgcolor?>" class="tabela_texto"><?=(($vobj[$i]["periodo_status"]==1) ? '<img src="images/icones/s1.gif" title="Ativo" class="status" />' : '<img src="images/icones/s0.gif" title="Inativo" class="status" />')?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=BannerTipo::_Descricao($vobj[$i]["tipo"])?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_banner"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="relatorio_banner_acesso.php?id=<?=$vobj[$i]["id_banner"]?>" target="_blank"><img src="menu/icones/fileprint.gif" alt="Relat&oacute;rio de Clicks" border="0" /></a>&nbsp;<a href="banner_view.php?id=<?=$vobj[$i]["id_banner"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_banner"]?>" /></a>&nbsp;<a href="banner_edit.php?id=<?=$vobj[$i]["id_banner"]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_banner"]?>" width="18" height="18" border="0"></a><a href="banner_remove.php?id=<?=$vobj[$i]["id_banner"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_banner"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
