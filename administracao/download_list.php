<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("download_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Download: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="download_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="15%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_download_categoria&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Download Categoria">Categoria</a></td>
        <td height="20" width="30%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=nome&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Nome">Nome</a></td>
        <td height="20" width="10%" align="center"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=click&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Click">Click</a></td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = DownloadCategoriaManage::buscarDownloadCategoria(1, $vobj[$i]["id_download_categoria"]);	print $value["nome"]; ?></td>
        <td align="left" bgcolor="<?=$bgcolor?>" class="tabela_texto"><?=System::FormatarStringList($vobj[$i]["nome"])?></td>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><?=System::FormatarStringList($vobj[$i]["click"])?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_download"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="download_priority.php?idatual=<?=$vobj[$i]["id_download"]?>&idalterar=<?=$vobj[$i-$h]["id_download"]?>&back=<?=$link_back;?>" onclick="javascript: return confirm('Deseja realmente Alterar a Prioridade?');"><img src="images/seta1.gif" width="15" height="15" border="0" alt="Alterar Prioridade | Código: <?=$vobj[$i]["id_download"]?> por <?=$vobj[$i-$h]["id_download"]?>" /></a>&nbsp;<a href="download_view.php?id=<?=$vobj[$i]["id_download"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_download"]?>" /></a>&nbsp;<a href="download_edit.php?id=<?=$vobj[$i]["id_download"]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_download"]?>" width="18" height="18" border="0"></a><a href="download_remove.php?id=<?=$vobj[$i]["id_download"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_download"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
