<?
	$link_add = "lojista_add.php";
	$link_edit = "lojista_edit.php";
	$link_remove = "lojista_remove.php";
	$link_list = "lojista_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = LojistaAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(LojistaAttribute::Nome(), SearchOrder::Crescente());
		$ordenacao_campo = LojistaAttribute::Nome();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("lojista_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = LojistaManage::count( SearchMounter::MounterByQueryCount(LojistaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = LojistaManage::consultar(SearchMounter::MounterByQueryLimit(LojistaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = LojistaAttribute::_GetAllAttributes();
?>