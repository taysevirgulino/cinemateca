<?
	$link_add = "associado_add.php";
	$link_edit = "associado_edit.php";
	$link_remove = "associado_remove.php";
	$link_list = "associado_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = AssociadoAttribute::Apelido();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(AssociadoAttribute::Apelido(), SearchOrder::Crescente());
		$ordenacao_campo = AssociadoAttribute::Apelido();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("associado_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = AssociadoManage::count( SearchMounter::MounterByQueryCount(AssociadoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = AssociadoManage::consultar(SearchMounter::MounterByQueryLimit(AssociadoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = AssociadoAttribute::_GetAllAttributes();
?>