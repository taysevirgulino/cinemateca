<?
	...
	$vobj = EixoManage::consultar(SearchMounter::MounterByQueryComparerOrder(EixoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Eixo")
								 ->setSubject("Dados Eixo")
								 ->setDescription("Dados Eixo")
								 ->setKeywords("Dados Eixo")
								 ->setCategory("Eixo");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Eixo"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Categoria"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Disciplina"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_eixo"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($vobj[$i]["categoria"]));
		...
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode($ValueDisciplina["nome"]));
	...
	$fileName = "Eixo_".date("d-m-Y_H-i-s").".xlsx";
