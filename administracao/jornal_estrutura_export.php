<?
	...
	$vobj = JornalEstruturaManage::consultarJornalEstrutura(3, SearchMounter::MounterByQueryComparerOrder(JornalEstruturaAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados JornalEstrutura")
								 ->setSubject("Dados JornalEstrutura")
								 ->setDescription("Dados JornalEstrutura")
								 ->setKeywords("Dados JornalEstrutura")
								 ->setCategory("JornalEstrutura");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Jornal Estrutura"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_jornal_estrutura"]));
	...
	$fileName = "JornalEstrutura_".date("d-m-Y_H-i-s").".xlsx";
