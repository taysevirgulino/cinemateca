<?
	...
	$vobj = ArquivoAnexoManage::consultarArquivoAnexo(3, SearchMounter::MounterByQueryComparerOrder(ArquivoAnexoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados ArquivoAnexo")
								 ->setSubject("Dados ArquivoAnexo")
								 ->setDescription("Dados ArquivoAnexo")
								 ->setKeywords("Dados ArquivoAnexo")
								 ->setCategory("ArquivoAnexo");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Arquivo Anexo"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("D1", utf8_encode("Arquivo Histórico"));
	$objPHPExcel->getActiveSheet()->getStyle("D1")->applyFromArray($styleTitulo);
	$objPHPExcel->getActiveSheet()->getColumnDimension("D")->setAutoSize(true);

	$objPHPExcel->getActiveSheet()->setCellValue("E1", utf8_encode("Arquivo"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_arquivo_anexo"]));
		...
		$ValueArquivoHistorico = ArquivoHistoricoManage::buscarArquivoHistorico(1, $vobj[$i]["id_arquivo_historico"]);
		$objPHPExcel->getActiveSheet()->setCellValue("D".($i+2), utf8_encode($ValueArquivoHistorico["texto"]));
		$objPHPExcel->getActiveSheet()->setCellValue("E".($i+2), utf8_encode($vobj[$i]["arquivo"]));
	...
	$fileName = "ArquivoAnexo_".date("d-m-Y_H-i-s").".xlsx";
