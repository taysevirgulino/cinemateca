<?
	...
	$vobj = EmktContatoManage::consultarEmktContato(3, SearchMounter::MounterByQueryComparerOrder(EmktContatoAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados EmktContato")
								 ->setSubject("Dados EmktContato")
								 ->setDescription("Dados EmktContato")
								 ->setKeywords("Dados EmktContato")
								 ->setCategory("EmktContato");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Emkt Contato"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_emkt_contato"]));
	...
	$fileName = "EmktContato_".date("d-m-Y_H-i-s").".xlsx";
