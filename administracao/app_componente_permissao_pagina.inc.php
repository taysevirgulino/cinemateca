<?
	...
	if ( ! Validacao::isVazio($frm_bt) ){
		...
		if( Validacao::isVazio($label_alerta_erro) ){
			...
			$vp = AppComponentePermissaoManage::consultarAppComponentePermissao(5, $ID);
		...
		$vobj = array();
		$vg = AppComponentePaginaManage::consultarAppComponentePagina(4, $ID);
		for($i=0; $i < count($vg); $i++){
			$vobj[$i] = $vg[$i]["id_app_componente_pagina"];
		}
	}
?>