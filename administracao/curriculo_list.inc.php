<?
	$link_add = "curriculo_add.php";
	$link_edit = "curriculo_edit.php";
	$link_remove = "curriculo_remove.php";
	$link_list = "curriculo_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = CurriculoAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(CurriculoAttribute::Nome(), SearchOrder::Crescente());
		$ordenacao_campo = CurriculoAttribute::Nome();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("curriculo_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = CurriculoManage::count( SearchMounter::MounterByQueryCount(CurriculoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = CurriculoManage::consultar(SearchMounter::MounterByQueryLimit(CurriculoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = CurriculoAttribute::_GetAllAttributes();
?>