<?
	$link_add = "audio_add.php";
	$link_edit = "audio_edit.php";
	$link_remove = "audio_remove.php";
	$link_list = "audio_list.php";
	...
	$SearchQueryComparerCollection[] = new SearchQueryComparer(AudioAttribute::IdeOrigem(), SearchComparer::Igual(), SearchCondition::E(), Current::IdeOrigem());
	if(empty($busca_campo)){
		$busca_campo = AudioAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(AudioAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = AudioAttribute::Titulo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("audio_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = AudioManage::count( SearchMounter::MounterByQueryCount(AudioAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = AudioManage::consultar(SearchMounter::MounterByQueryLimit(AudioAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = AudioAttribute::_GetAllAttributes();
?>