<?
	...
	$vobj = ArquivoDownloadManage::consultarArquivoDownload(3, SearchMounter::MounterByQueryComparerOrder(ArquivoDownloadAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados ArquivoDownload")
								 ->setSubject("Dados ArquivoDownload")
								 ->setDescription("Dados ArquivoDownload")
								 ->setKeywords("Dados ArquivoDownload")
								 ->setCategory("ArquivoDownload");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Arquivo Download"));
	...
	$objPHPExcel->getActiveSheet()->setCellValue("C1", utf8_encode("Arquivo Anexo"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_arquivo_download"]));
		$objPHPExcel->getActiveSheet()->setCellValue("B".($i+2), utf8_encode($vobj[$i]["identificador"]));
		$ValueArquivoAnexo = ArquivoAnexoManage::buscarArquivoAnexo(1, $vobj[$i]["id_arquivo_anexo"]);
		$objPHPExcel->getActiveSheet()->setCellValue("C".($i+2), utf8_encode($ValueArquivoAnexo["arquivo"]));
	...
	$fileName = "ArquivoDownload_".date("d-m-Y_H-i-s").".xlsx";
