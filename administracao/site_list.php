<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("site_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Site: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="site_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="70%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=titulo&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Título">Título</a></td>
      ...
      <tr>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_site"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="site_view.php?id=<?=$vobj[$i]["id_site"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_site"]?>" /></a>&nbsp;<a href="site_edit.php?id=<?=$vobj[$i]["id_site"]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_site"]?>" width="18" height="18" border="0"></a><a href="site_remove.php?id=<?=$vobj[$i]["id_site"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_site"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
