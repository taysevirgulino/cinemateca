<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("curriculo_acesso_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Currículo Acesso: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdCurriculo">&nbsp;Currículo: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdCurriculo" class="frm" id="FrmIdCurriculo" style="width:500px;">
    <? for($i=0; $i < count($VObjCurriculo); $i++){ ?>
    <option value="<?=$VObjCurriculo[$i]["id_curriculo"]?>" <? if($VObjCurriculo[$i]["id_curriculo"] == $frm_id_curriculo){ print("selected"); }?>><?=$VObjCurriculo[$i]["nome"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
    ...
    <script language="javascript">document.getElementById("FrmIdCurriculo").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdCurriculo':{'l':' Currículo','t':'labelFrmIdCurriculo','r':true},
