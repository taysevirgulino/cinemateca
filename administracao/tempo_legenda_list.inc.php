<?
	$link_add = "tempo_legenda_add.php";
	$link_edit = "tempo_legenda_edit.php";
	$link_remove = "tempo_legenda_remove.php";
	$link_list = "tempo_legenda_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = TempoLegendaAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(TempoLegendaAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = TempoLegendaAttribute::Titulo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("tempo_legenda_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = SearchMysqlQuery::CountBySql( SearchMounter::MounterByQueryCount(TempoLegendaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = TempoLegendaManage::consultarTempoLegenda(3, SearchMounter::MounterByQueryLimit(TempoLegendaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = TempoLegendaAttribute::_GetAllAttributes();
?>