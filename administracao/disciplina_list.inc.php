<?
	$link_add = "disciplina_add.php";
	$link_edit = "disciplina_edit.php";
	$link_remove = "disciplina_remove.php";
	$link_list = "disciplina_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = DisciplinaAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(DisciplinaAttribute::Nome(), SearchOrder::Crescente());
		$ordenacao_campo = DisciplinaAttribute::Nome();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("disciplina_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = DisciplinaManage::count( SearchMounter::MounterByQueryCount(DisciplinaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = DisciplinaManage::consultar(SearchMounter::MounterByQueryLimit(DisciplinaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = DisciplinaAttribute::_GetAllAttributes();
?>