<?
	...
	$vobj = AgendaCategoriaManage::consultarAgendaCategoria(3, SearchMounter::MounterByQueryComparerOrder(AgendaCategoriaAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AgendaCategoria")
								 ->setSubject("Dados AgendaCategoria")
								 ->setDescription("Dados AgendaCategoria")
								 ->setKeywords("Dados AgendaCategoria")
								 ->setCategory("AgendaCategoria");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Agenda Categoria"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_agenda_categoria"]));
	...
	$fileName = "AgendaCategoria_".date("d-m-Y_H-i-s").".xlsx";
