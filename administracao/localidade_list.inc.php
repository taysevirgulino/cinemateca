<?
	$link_add = "localidade_add.php";
	$link_edit = "localidade_edit.php";
	$link_remove = "localidade_remove.php";
	$link_list = "localidade_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = LocalidadeAttribute::Uf();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(LocalidadeAttribute::Uf(), SearchOrder::Crescente());
		$ordenacao_campo = LocalidadeAttribute::Uf();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("localidade_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = SearchMysqlQuery::CountBySql( SearchMounter::MounterByQueryCount(LocalidadeAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = LocalidadeManage::consultarLocalidade(3, SearchMounter::MounterByQueryLimit(LocalidadeAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = LocalidadeAttribute::_GetAllAttributes();
?>