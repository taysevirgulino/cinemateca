<?
	...
	$vobj = LinkManage::consultarLink(3, SearchMounter::MounterByQueryComparerOrder(LinkAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados Link")
								 ->setSubject("Dados Link")
								 ->setDescription("Dados Link")
								 ->setKeywords("Dados Link")
								 ->setCategory("Link");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Link"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_link"]));
	...
	$fileName = "Link_".date("d-m-Y_H-i-s").".xlsx";
