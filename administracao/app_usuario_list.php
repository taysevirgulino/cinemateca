<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("app_usuario_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> App Usuário: Listagem </td>
  </tr>
  <tr>
  ...
  <tr>
    <td>
	<form id="frm" name="frm" method="get" action="app_usuario_list.php">
      <table width="100%" border="0">
        ...
        <tr>
          <td colspan="4" align="right" height="25"><input name="pesquisar" type="submit" class="frm_botao" id="pesquisar" value="Pesquisar" />
            <input name="cancelar" type="button" class="frm_botao" id="cancelar" value="Cancelar" onclick="javascript:window.open('app_usuario_list.php', '_parent');" /></td>
        </tr>
		<tr>
  ...
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="20%"> Grupo</td>
        <td height="20" width="20%"> Nome</td>
        <td height="20" width="20%"> E-mail</td>
        <td height="20" width="20%"> Login</td>
        <td height="20" width="10%" align="center"> Status</td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = AppUsuarioGrupoManage::buscarAppUsuarioGrupo(1, $vobj[$i][2]);	print $value["nome"]; ?></td>
        ...
        <td align="left" bgcolor="<?=$bgcolor?>" class="tabela_texto"><?=System::FormatarStringList($vobj[$i][5])?></td>
        ...
        <td height="25" bgcolor="#CCCCCC" align="center"><a href="app_usuario_view.php?id=<?=$vobj[$i][0]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i][0]?>" /></a>&nbsp;<a href="app_usuario_edit.php?id=<?=$vobj[$i][0]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i][0]?>" width="18" height="18" border="0"></a><a href="app_usuario_remove.php?id=<?=$vobj[$i][0]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i][0]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
      ?>
	  <tr>
	  	<td colspan="6" style="height:30px; font-weight:bold; font-size:14px; text-align:center">Nenhum registro afetado!</td>
	  </tr>
	  <?
	  	}
	  ?>
