<?
	$link_add = "mensagem_resposta_add.php";
	$link_edit = "mensagem_resposta_edit.php";
	$link_remove = "mensagem_resposta_remove.php";
	$link_list = "mensagem_resposta_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = MensagemRespostaAttribute::Texto();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(MensagemRespostaAttribute::Texto(), SearchOrder::Crescente());
		$ordenacao_campo = MensagemRespostaAttribute::Texto();
	...
	if(!Validacao::isVazio($exportar)){
		...
		$export = array();
		$export["Object"] = "MensagemResposta";
		...
		System::Redirect("mensagem_resposta_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = MensagemRespostaManage::count( SearchMounter::MounterByQueryCount(MensagemRespostaAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = MensagemRespostaManage::consultar(SearchMounter::MounterByQueryLimit(MensagemRespostaAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = MensagemRespostaAttribute::_GetAllAttributes();
?>