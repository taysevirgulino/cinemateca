<?
	$link_add = "empreendimento_add.php";
	$link_edit = "empreendimento_edit.php";
	$link_remove = "empreendimento_remove.php";
	$link_list = "empreendimento_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = EmpreendimentoAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(EmpreendimentoAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = EmpreendimentoAttribute::Titulo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("empreendimento_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = EmpreendimentoManage::count( SearchMounter::MounterByQueryCount(EmpreendimentoAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = EmpreendimentoManage::consultar(SearchMounter::MounterByQueryLimit(EmpreendimentoAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = EmpreendimentoAttribute::_GetAllAttributes();
?>