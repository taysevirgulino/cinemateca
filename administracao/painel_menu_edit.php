<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("painel_menu_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Painel Menu: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdPainelMenuPai">&nbsp;Menu: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdPainelMenuPai" class="frm" id="FrmIdPainelMenuPai" style="width:500px;">
          	    <option value="0" selected="selected">[sem menu]</option>
                <? for($i=0; $i < count($VObjPainelMenu); $i++){ ?>
                <option value="<?=$VObjPainelMenu[$i][1]?>" <? if($VObjPainelMenu[$i][1] == $frm_id_painel_menu_pai){ print("selected"); }?>><?=$VObjPainelMenu[$i][0]?></option>
                <? } ?>
              </select>
          </td>
        </tr>
        ...
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmStyle">&nbsp;Style:  </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="text" name="FrmStyle" id="FrmStyle" class="frm" size="80" maxlength="255" value="<?=$frm_style?>">
          </td>
        </tr>
        <tr>
        ...
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmUsuarioPerfil">&nbsp;Perfil de Usuário:  </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<?php 
          	     foreach ($VObjUsuarioPerfil as $key => $value) {
          	         print sprintf('<div style="position:relative; float:left; width:100%%; padding:5px 5px 0px 0px;"><label><input type="checkbox" name="FrmIdUsuarioPerfil[]" id="FrmIdUsuarioPerfil_%1$s" value="%1$s" /> %2$s</label></div>', $value["id_usuario_perfil"], $value["titulo"]);
          	     }
          	?>
          </td>
        </tr>
    ...
    <script language="javascript">document.getElementById("FrmIdPainelMenuPai").focus();</script>
	<script language="javascript">
    <!--
        <?php 
            foreach ($itensPainelMenuPerfil as $valuePainelMenuPerfil) {
                print sprintf(
                    '$("#FrmIdUsuarioPerfil_%1$s").attr("checked", "checked"); ',
                    $valuePainelMenuPerfil["id_usuario_perfil"]
                );
            }
        ...
        var a_fields = {
            'FrmIdPainelMenuPai':{'l':' Painel Menu','t':'labelFrmIdPainelMenuPai'},
            'FrmNome':{'l':' Nome','t':'labelFrmNome','r':true},
            'FrmUrl':{'l':' Url (Endereço)','t':'labelFrmUrl','r':true},
            'FrmStyle':{'l':' Style','t':'labelFrmStyle'},
            'FrmTarget':{'l':' Abrir','t':'labelFrmTarget'},
