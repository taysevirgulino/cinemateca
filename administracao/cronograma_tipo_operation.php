			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						CronogramaTipoManage::excluirCronogramaTipo($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new CronogramaTipo();
						if(!$obj_atual->buscarCronogramaTipo(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new CronogramaTipo();
						if(!$obj_alterar->buscarCronogramaTipo(1, $itens[1])){ System::Redirect($link_list); }
				...
				case "status" : {
					for ($i=0; $i < count($itens); $i++){
						...
						if($obj->buscarCronogramaTipo(1, $itens[$i])){
