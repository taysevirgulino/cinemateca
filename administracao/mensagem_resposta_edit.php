<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("mensagem_resposta_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Mensagem Resposta: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdMensagem">&nbsp;Mensagem: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdMensagem" class="frm" id="FrmIdMensagem" style="width:500px;">
    <? for($i=0; $i < count($VObjMensagem); $i++){ ?>
    <option value="<?=$VObjMensagem[$i]["id_mensagem"]?>" <? if($VObjMensagem[$i]["id_mensagem"] == $frm_id_mensagem){ print("selected"); }?>><?=$VObjMensagem[$i]["titulo"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
          ...
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	<input type="file" name="FrmArquivoFile" id="FrmArquivoFile" class="frm" size="60" maxlength="255" value="">          	<? if(!empty($frm_arquivo)){ ?>
          		<div style="position:relative; float:left; width:100%; margin:5px 0px 0px 0px;">
          			<a href="../files/mensagem_resposta/<?=$frm_arquivo?>" target="_blank" style="text-decoration:underline;"><?=$frm_arquivo?></a>
    ...
    <script language="javascript">document.getElementById("FrmIdMensagem").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdMensagem':{'l':' Mensagem','t':'labelFrmIdMensagem','r':true},
