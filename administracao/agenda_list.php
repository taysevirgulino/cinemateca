<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("agenda_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Agenda: Listagem </td>
  </tr>
  <tr>
      ...
      <table width="100%" border="0">
        ...
        <tr>
          <td colspan="4" align="right" height="25"><input name="pesquisar" type="submit" class="frm_botao" id="pesquisar" value="Pesquisar" />
            <input name="cancelar" type="button" class="frm_botao" id="cancelar" value="Cancelar" onclick="javascript:window.open('agenda_list.php', '_parent');" />
            <input name="exportar" id="exportar" type="submit" class="frm_botao frm_botao_export" value="Exportar" /></td>
        </tr>
		<tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="agenda_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="15%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_agenda_categoria&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Agenda Categoria">Categoria</a></td>
        <td height="20" width="40%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=titulo&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Título">Título</a></td>
        <td height="20" width="15%" align="left"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=data&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Data">Data e Hora</a></td>
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = AgendaCategoriaManage::buscarAgendaCategoria(1, $vobj[$i]["id_agenda_categoria"]);	print $value["nome"]; ?></td>
        <td align="left" bgcolor="<?=$bgcolor?>" class="tabela_texto"><?=System::FormatarStringList($vobj[$i]["titulo"])?></td>
        <td align="left" bgcolor="<?=$bgcolor?>" class="tabela_texto"><?=System::FormatarData($vobj[$i]["data"], "d/m/y");?> às <?=System::FormatarData($vobj[$i]["hora"], "H:i")?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_agenda"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="<?=Url::Agenda($vobj[$i]["id_agenda"], $vobj[$i]["identificador"], $vobj[$i]["titulo"])?>" target="_blank"><img src="images/icones/world_link.gif" border="0" title="Visualizar no Site" /></a>&nbsp;<a href="agenda_view.php?id=<?=$vobj[$i]["id_agenda"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" title="Visualizar | Código: <?=$vobj[$i]["id_agenda"]?>" /></a>&nbsp;<a href="agenda_edit.php?id=<?=$vobj[$i]["id_agenda"]?>"><img src="images/botao/Edit.gif" title="Editar | Código: <?=$vobj[$i]["id_agenda"]?>" width="18" height="18" border="0"></a><a href="agenda_remove.php?id=<?=$vobj[$i]["id_agenda"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" title="Excluir | Código: <?=$vobj[$i]["id_agenda"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
