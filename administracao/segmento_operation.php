			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						SegmentoManage::excluirSegmento($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new Segmento();
						if(!$obj_atual->buscarSegmento(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new Segmento();
						if(!$obj_alterar->buscarSegmento(1, $itens[1])){ System::Redirect($link_list); }
