<?
	...
	if(empty($busca_campo)){
		$busca_campo = BannerLocalAttribute::Nome();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(BannerLocalAttribute::Nome(), SearchOrder::Crescente());
		$ordenacao_campo = BannerLocalAttribute::Nome();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("banner_local_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = BannerLocalManage::count( SearchMounter::MounterByQueryCount(BannerLocalAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = BannerLocalManage::consultar(SearchMounter::MounterByQueryLimit(BannerLocalAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = BannerLocalAttribute::_GetAllAttributes();
?>