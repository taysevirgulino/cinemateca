			...
			switch ($op_opcao){
				case "excluir" : {
					for ($i=0; $i < count($itens); $i++){
						AgendaCategoriaManage::excluirAgendaCategoria($itens[$i]);
					}
				}break;
				case "prioridade" : {
					if(count($itens) >= 2){
						$obj_atual = new AgendaCategoria();
						if(!$obj_atual->buscarAgendaCategoria(1, $itens[0])){ System::Redirect($link_list); }

						$obj_alterar = new AgendaCategoria();
						if(!$obj_alterar->buscarAgendaCategoria(1, $itens[1])){ System::Redirect($link_list); }
