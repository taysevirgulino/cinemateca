<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("bloco_menu_edit.inc.php"); require_once("script/editorhtml/ckeditor.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Bloco Menu: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdBlocoMenuPai">&nbsp;Menu: </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdBlocoMenuPai" class="frm" id="FrmIdBlocoMenuPai" style="width:500px;">
              <option value="0">[nenhum]</option>
    <? for($i=0; $i < count($VObjBlocoMenu); $i++){ ?>
    <option value="<?=$VObjBlocoMenu[$i][0]?>" <? if($VObjBlocoMenu[$i][0] == $frm_id_bloco_menu_pai){ print("selected"); }?>><?=$VObjBlocoMenu[$i]["heranca"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
    ...
    <script language="javascript">document.getElementById("FrmIdBlocoMenuPai").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdBlocoMenuPai':{'l':' Bloco Menu','t':'labelFrmIdBlocoMenuPai','r':true},
            'FrmNome':{'l':' Nome','t':'labelFrmNome','r':true},
            'FrmUrl':{'l':' Url (Endereço)','t':'labelFrmUrl'},
