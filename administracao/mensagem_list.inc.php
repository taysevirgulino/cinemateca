<?
	$link_add = "mensagem_add.php";
	$link_edit = "mensagem_edit.php";
	$link_remove = "mensagem_remove.php";
	$link_list = "mensagem_list.php";
	...
	if(empty($busca_campo)){
		$busca_campo = MensagemAttribute::Titulo();
	}
	if(!empty($chave)){
	...
	if(!empty($ordenacao_campo)){
		...
		$SearchQueryOrderCollection[] = new SearchQueryOrder(MensagemAttribute::Titulo(), SearchOrder::Crescente());
		$ordenacao_campo = MensagemAttribute::Titulo();
	...
	if(!Validacao::isVazio($exportar)){
		...
		System::Redirect("mensagem_export.php?ide=$export_ide");
	}

	// Paginчуo de resultados ----------------------------------------------------------|
	$vobjCount = MensagemManage::count( SearchMounter::MounterByQueryCount(MensagemAttribute::_Table(), $SearchQueryComparerCollection) );
	...
	$vobj = MensagemManage::consultar(SearchMounter::MounterByQueryLimit(MensagemAttribute::_Table(), $SearchQueryComparerCollection, $SearchQueryOrderCollection, $paginacao->getPaginacaoInicio(), $paginacao->getRegistrosPorPagina()));

	$Orders = SearchOrder::_GetAllOrders();
	$Comparers = SearchComparer::_GetAllComparers();
	$Attributes = MensagemAttribute::_GetAllAttributes();
?>