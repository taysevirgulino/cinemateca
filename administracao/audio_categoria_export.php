<?
	...
	$vobj = AudioCategoriaManage::consultarAudioCategoria(3, SearchMounter::MounterByQueryComparerOrder(AudioCategoriaAttribute::_Table(), $export["SearchQueryComparerCollection"], $export["SearchQueryOrderCollection"]));

	$objPHPExcel = new PHPExcel();
	$objPHPExcel->getProperties()->setCreator("ArtemSite")
								 ->setLastModifiedBy("ArtemSite")
								 ->setTitle("Dados AudioCategoria")
								 ->setSubject("Dados AudioCategoria")
								 ->setDescription("Dados AudioCategoria")
								 ->setKeywords("Dados AudioCategoria")
								 ->setCategory("AudioCategoria");
	...
	$objPHPExcel->getActiveSheet()->setCellValue("A1", utf8_encode("Código Áudio Categoria"));
	...
	for ($i = 0; $i < count($vobj); $i++) {
		$objPHPExcel->getActiveSheet()->setCellValue("A".($i+2), utf8_encode($vobj[$i]["id_audio_categoria"]));
	...
	$fileName = "AudioCategoria_".date("d-m-Y_H-i-s").".xlsx";
