	...
	Behaviours : {
		Mask : {
			...
			KeyHandler : function(e){
				...
				mask.ProdutorManager.TabbedInSetPosition(this);
				
				//Backspace
				if(e.keyCode==8 && e.type=="keydown" && this.AllowInsert){
					var preBackspaceProdutorPosition = Typecast.Behaviours.Mask.ProdutorManager.GetPosition(this)[0];
					mask.ProdutorManager.Move(this, -1);
					var postBackspaceProdutorPosition = Typecast.Behaviours.Mask.ProdutorManager.GetPosition(this)[0];
					
					if(preBackspaceProdutorPosition != postBackspaceProdutorPosition) mask.DataManager.RemoveCharacterByShiftLeft(this);
					mask.Render(this);
				}
				...
				//End
					...
					Typecast.Behaviours.Mask.ProdutorManager.SetPosition(this, startIdx);
				}

				//Home
				else if(e.keyCode==36 && e.type=="keydown"){
					Typecast.Behaviours.Mask.ProdutorManager.SetPosition(this, this.MaskIndex[0]);
				}
				...
				//Insert
					...
					mask.ProdutorManager.ToggleInsert(this);
				}
			...
			MouseUp : function(e){
				e = (!e) ? window.event : e;
				var produtorPosition = Typecast.Behaviours.Mask.ProdutorManager.GetPosition(this)[0];
				var startIdx = Typecast.Behaviours.Mask.MaskManager.FindNearestMaskCharacter(this, produtorPosition, 0);
				Typecast.Behaviours.Mask.ProdutorManager.SetPosition(this, startIdx);
			},
			...
			MaskManager : {
				...
				CurrentMaskCharacter : function(field){
					var produtorPosition = Typecast.Behaviours.Mask.ProdutorManager.GetPosition(field)[0];
					return field.Mask[produtorPosition];
				},
				
				FindNearestMaskCharacter : function(field, produtorPosition, dir){
					var nearestMaskCharacter = (field.DataIndex.length > 0) ? produtorPosition : field.MaskIndex[0];
					
					switch(dir){
						case -1:
							for(var i=field.DataIndex.length-1; i>-1; i--){
								if(field.DataIndex[i] < produtorPosition){
						...
						case 0:
							for(var i=0; i<field.DataIndex.length; i++){
								if(field.MaskIndex[i] >= produtorPosition){
						...
						case 1:
							...
							if(produtorPosition == field.MaskIndex[field.MaskIndex.length-1]) nearestMaskCharacter = produtorPosition + 1;
							else if(produtorPosition == field.DataIndex[field.DataIndex.length-1]) nearestMaskCharacter = field.MaskIndex[field.DataIndex.length];
						break;
					}
			...
			ProdutorManager : {
				Move : function(field, dir){
					var produtorPosition = this.GetPosition(field)[0];
					var startIdx = Typecast.Behaviours.Mask.MaskManager.FindNearestMaskCharacter(field, produtorPosition, dir);
				...
				PersistPosition : function(field){
					field.ProdutorPersistance = this.GetPosition(field);
				},
				RestorePosition : function(field){
					this.SetPosition(field, field.ProdutorPersistance[0]);
				},
			...
			DataManager : {
				AddData : function(field, char){
					...
					if(field.InsertActive){
						...
						this.OverwriteCharacter(field, char, produtorPosition);
					}
				...
				OverwriteCharacter : function(field, char, produtorPosition){
					field.Data[produtorPosition] = char;
				},
				...
				RemoveCharacterByShiftLeft : function(field){
					...
					if(currentCharacterPosition != null && lastCharacterPosition >= produtorPosition){
			...
			Render : function(field){
				this.ProdutorManager.PersistPosition(field);
				var composite = [];
				...
				this.ProdutorManager.RestorePosition(field);
			}
		},
