	...
	var commandObject =
	{
		exec : function( editor )
		{
			...
			if ( CKEDITOR.env.ie )
			{
				...
				{
					produtor = bookmarkStart;
					while ( ( produtor = produtor.getNext() ) )
					{
						if ( produtor.type == CKEDITOR.NODE_ELEMENT &&
								produtor.isBlockBoundary() )
						{
							bookmarkStart.move( produtor, true );
							break;
						}
					}
				}
				...
				{
					produtor = bookmarkEnd;
					while ( ( produtor = produtor.getPrevious() ) )
					{
						if ( produtor.type == CKEDITOR.NODE_ELEMENT &&
								produtor.isBlockBoundary() )
						{
							bookmarkEnd.move( produtor );
							break;
						}
					}
				}
