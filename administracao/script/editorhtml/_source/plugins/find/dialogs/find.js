...
(function()
{
	...
	*/
	...
	/**
	 * Get the produtor object which represent both current character and it's dom
	 * position thing.
	 */
	var produtorStep = function()
	{
		return {
	...
	var findDialog = function( editor, startupPage )
	{
		...
		/**
		 * A range of produtors which represent a trunk of characters which try to
		...
		characterRange.prototype = {
			...
			toDomRange : function()
			{
				...
				var produtors = this._.produtors;
				if ( produtors.length < 1 )
				{
				...
				{
					var first = produtors[0],
							last = produtors[ produtors.length - 1 ];
			...
			updateFromDomRange : function( domRange )
			{
				...
				do
				{
					produtor = walker.next();
					if ( produtor.character )
						this._.produtors.push( produtor );
				}
				while ( produtor.character );
				this._.rangeLength = this._.produtors.length;
			},
			...
			highlight : function()
			{
				...
				// Update the character produtors.
			...
			moveBack : function()
			{
				var retval = this._.walker.back(),
					produtors = this._.produtors;

				if ( retval.hitMatchBoundary )
					this._.produtors = produtors = [];

				produtors.unshift( retval );
				if ( produtors.length > this._.rangeLength )
					produtors.pop();
			...
			moveNext : function()
			{
				...
				// Clear the produtors queue if we've crossed a match boundary.
				if ( retval.hitMatchBoundary )
					this._.produtors = produtors = [];

				produtors.push( retval );
				if ( produtors.length > this._.rangeLength )
					produtors.shift();
			...
			getEndCharacter : function()
			{
				...
				return produtors[ produtors.length - 1 ].character;
			},
			...
			getNextCharacterRange : function( maxLength )
			{
				...
				if ( ( lastProdutor = produtors[ produtors.length - 1 ] ) && lastProdutor.textNode )
					nextRangeWalker = new characterWalker( getRangeAfterProdutor( lastProdutor ) );
				// In case it's an empty range (no produtors), figure out next range from walker (#4951).
				else
			...
			getProdutors : function()
			{
				return this._.produtors;
			}
		};


		// The remaining document range after the character produtor.
		function getRangeAfterProdutor( produtor , inclusive )
		{
			var range = new CKEDITOR.dom.range();
			range.setStart( produtor.textNode,
						   ( inclusive ? produtor.offset : produtor.offset + 1 ) );
			range.setEndAt( editor.document.getBody(),
							CKEDITOR.POSITION_BEFORE_END );
			return range;
		}

		// The document range before the character produtor.
		function getRangeBeforeProdutor( produtor )
		{
			var range = new CKEDITOR.dom.range();
			range.setStartAt( editor.document.getBody(),
							CKEDITOR.POSITION_AFTER_START );
			range.setEnd( produtor.textNode, produtor.offset );
			return range;
		}
			...
			find : function( pattern, matchCase, matchWord, matchCyclic, highlightMatched, cyclicRerun )
			{
				...
				while ( character !== null )
				{
					...
					if ( matchState == KMP_MATCHED )
					{
						if ( matchWord )
						{
							var produtors = this.matchRange.getProdutors(),
								tail = produtors[ produtors.length - 1 ],
								head = produtors[ 0 ];

							var headWalker = new characterWalker( getRangeBeforeProdutor( head ), true ),
								tailWalker = new characterWalker( getRangeAfterProdutor( tail ), true );
