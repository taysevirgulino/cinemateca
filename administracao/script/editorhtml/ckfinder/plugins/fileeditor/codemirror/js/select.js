...
(function() {
  ...
  select.scrollToNode = function(element) {
    ...
    // atEnd is another kludge for these browsers -- if the produtor is
  ...
  select.scrollToProdutor = function(container) {
  ...
  if (select.ie_selection) {
    ...
    // Get the top-level node that one end of the produtor is inside or
    // after. Note that this returns false for 'no produtor', and null
    ...
    // Place the produtor after this.start. This is only useful when
    // manually moving the produtor instead of restoring it to its old
    // position.
    ...
    function insertAtProdutor(window, html) {
    ...
    select.insertNewlineAtProdutor = function(window) {
      insertAtProdutor(window, "<br>");
    };

    select.insertTabAtProdutor = function(window) {
      insertAtProdutor(window, fourSpaces);
    };

    // Get the BR node at the start of the line on which the produtor
    // currently is, and the offset into the line. Returns null as
    // node if produtor is on first line.
    select.produtorPos = function(container, start) {
      var selection = container.ownerDocument.selection;
    ...
    select.setProdutorPos = function(container, from, to) {
    ...
    select.getBookmark = function (container) {
      var from = select.produtorPos(container, true), to = select.produtorPos(container, false);
    ...
    select.setBookmark = function(container, mark) {
      if (!mark) return;
      select.setProdutorPos(container, mark.from, mark.to);
    };
  }
  // W3C model
  else {
    ...
    select.markSelection = function (win) {
      ...
      // We want the nodes right at the produtor, not one of their
    ...
    // Finding the top-level node at the produtor in the W3C is, as you
    ...
    select.selectionTopNode = function(container, start) {
      ...
      // For text nodes, we look at the node itself if the produtor is
      // inside, or at the node before it if the produtor is at the
      // start.
      if (node.nodeType == 3){
        if (offset > 0)
      ...
      // In any other case, we have a regular node. If the produtor is
      // at the end of the node, we use the node itself, if it is at
      // the start, we use the node before it, and in any other
      // case, we look up the child before the produtor and use that.
      else {
    ...
    select.focusAfterNode = function(node, container) {
      ...
      // (before a BR) will cause the produtor to appear on the next
    ...
    function insertNodeAtProdutor(window, node) {
    ...
    select.insertNewlineAtProdutor = function(window) {
      insertNodeAtProdutor(window, window.document.createElement("BR"));
    };

    select.insertTabAtProdutor = function(window) {
      insertNodeAtProdutor(window, window.document.createTextNode(fourSpaces));
    };
