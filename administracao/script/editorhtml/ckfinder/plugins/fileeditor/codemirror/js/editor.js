...
// behind it, the produtor can not be put on the line after it. This
...
var Editor = (function(){
  ...
  function traverseDOM(start){
    ...
    // between two BRs, because Opera's produtor code gets terribly
    // confused when the produtor is between two BRs.
    var afterBR = true;
  ...
  // these by calling CodeMirror.getSearchProdutor. To use, call
  // findNext on the resulting object -- this returns a boolean
  // indicating whether anything was found, and can be called again to
  // skip to the next find. Use the select and replace methods to
  // actually do something with the found locations.
  function SearchProdutor(editor, string, fromProdutor, caseFold) {
    this.editor = editor;
    ...
    var produtor;
    // Start from the produtor when specified and a produtor can be found.
    if (fromProdutor && (produtor = select.produtorPos(this.editor.container))) {
      this.line = produtor.node;
      this.offset = produtor.offset;
    }
    else {
      this.line = null;
      this.offset = 0;
  ...
  SearchProdutor.prototype = {
    findNext: function() {
      ...
      // Set the produtor's position one character after the given
      // position.
    ...
    select: function() {
      if (this.atOccurrence) {
        select.setProdutorPos(this.editor.container, this.atOccurrence.from, this.atOccurrence.to);
        select.scrollToProdutor(this.editor.container);
      }
    },
  ...
  function Editor(options) {
    ...
    if (!options.readOnly)
      select.setProdutorPos(container, {node: null, offset: 0});
    ...
    if (!options.readOnly) {
      ...
      function produtorActivity() {self.produtorActivity(false);}
      addEventHandler(document.body, "mouseup", produtorActivity);
      addEventHandler(document.body, "cut", produtorActivity);

      // workaround for a gecko bug [?] where going forward and then
      // back again breaks designmode (no more produtor)
      if (gecko)
      ...
      addEventHandler(document.body, "paste", function(event) {
        ...
        if (text !== null) {
          ...
          select.scrollToProdutor(self.container);
        }
      });
  ...
  Editor.prototype = {
    ...
    produtorPosition: function(start) {
      if (start == null) start = true;
      var pos = select.produtorPos(this.container, start);
    ...
    selectLines: function(startLine, startOffset, endLine, endOffset) {
      ...
      select.setProdutorPos(this.container, start, end);
      select.scrollToProdutor(this.container);
    },
    ...
    selectedText: function() {
      ...
      var start = select.produtorPos(this.container, true),
          end = select.produtorPos(this.container, false);
      if (!start || !end) return "";
    ...
    replaceSelection: function(text) {
      ...
      select.setProdutorPos(this.container, end);
      webkitLastLineHack(this.container);
    },
    ...
    getSearchProdutor: function(string, fromProdutor, caseFold) {
      return new SearchProdutor(this, string, fromProdutor, caseFold);
    },
    ...
    reindentSelection: function(direction) {
      if (!select.somethingSelected(this.win)) {
        this.indentAtProdutor(direction);
      }
      else {
    ...
    keyDown: function(event) {
      ...
      if (code == 13) { // enter
        if (event.ctrlKey && !event.altKey) {
          ...
          select.insertNewlineAtProdutor(this.win);
          this.indentAtProdutor();
          select.scrollToProdutor(this.container);
        }
        event.stop();
      }
        ...
        var produtor = select.selectionTopNode(this.container);
        if (produtor === false || !this.container.firstChild) return;

        if (code == 37) select.focusAfterNode(startOfLine(produtor), this.container);
        else {
          var end = endOfLine(produtor, this.container);
    ...
    keyPress: function(event) {
      ...
      if ((this.frozen && (!this.keyFilter || this.keyFilter(event.keyCode))) ||
        ...
        this.parent.setTimeout(function(){self.indentAtProdutor(null);}, 0);
      else if ((event.character == "v" || event.character == "V")
               && (event.ctrlKey || event.metaKey) && !event.altKey) // ctrl-V
        this.reroutePasteEvent();
    },

    // Mark the node at the produtor dirty when a non-safe key is
    // released.
    keyUp: function(event) {
      this.produtorActivity(isSafeKey(event.keyCode));
    },
    ...
    // re-indented, when nothing is selected, the line with the produtor
    // is re-indented.
    handleTab: function(direction) {
      if (this.options.tabMode == "spaces")
        select.insertTabAtProdutor(this.win);
      else
        this.reindentSelection(direction);
    },

    // Custom home behaviour that doesn't land the produtor in front of
    ...
    // Take the token before the produtor. If it contains a character in
    ...
    highlightParens: function(jump, fromKey) {
      ...
      var ch, produtor = select.selectionTopNode(this.container, true);
      if (!produtor || !this.highlightAtProdutor()) return;
      produtor = select.selectionTopNode(this.container, true);
      if (!(produtor && ((ch = paren(produtor)) || (produtor = produtor.nextSibling) && (ch = paren(produtor)))))
        return;
      // We only look for tokens with the same className.
      var className = produtor.className, dir = forward(ch), match = matching[ch];
      ...
      function tryFindMatch() {
        ...
        for (var runner = produtor; runner; runner = dir ? runner.nextSibling : runner.previousSibling) {
      ...
      while (true) {
        ...
        if (found.status == "dirty") {
          ...
          highlight(produtor, found.status);
          highlight(found.node, found.status);
          if (fromKey)
            self.parent.setTimeout(function() {unhighlight(produtor); unhighlight(found.node);}, 500);
          else
            self.highlighted = [produtor, found.node];
          if (jump && found.node)
    ...
    // the produtor is on so that it is indented properly.
    indentAtProdutor: function(direction) {
      if (!this.container.firstChild) return;
      // The line has to have up-to-date lexical information, so we
      // highlight it first.
      if (!this.highlightAtProdutor()) return;
      var produtor = select.selectionTopNode(this.container, false);
      // If we couldn't determine the place of the produtor,
      // there's nothing to indent.
      if (produtor === false)
        return;
      var lineStart = startOfLine(produtor);
      var whiteSpace = this.indentLineAfter(lineStart, direction);
      if (produtor == lineStart && whiteSpace)
          produtor = whiteSpace;
      // This means the indentation has probably messed up the produtor.
      if (produtor == whiteSpace)
        select.focusAfterNode(produtor, this.container);
    },
    ...
    indentRegion: function(start, end, direction) {
      ...
      select.setProdutorPos(this.container, {node: start, offset: 0}, {node: end, offset: 0});
    },

    // Find the node that the produtor is in, mark it as dirty, and make
    // sure a highlight pass is scheduled.
    produtorActivity: function(safe) {
      ...
      var activity = this.options.produtorActivity;
      if (!safe || activity) {
        var produtor = select.selectionTopNode(this.container, false);
        if (produtor === false || !this.container.firstChild) return;
        produtor = produtor || this.container.firstChild;
        if (activity) activity(produtor);
        if (!safe) {
          this.scheduleHighlight();
          this.addDirtyNode(produtor);
        }
      }
    },
    ...
    highlight: function(from, target, cleanLines, maxBacktrack){
      ...
      var parts = {
        ...
        getNonEmpty: function(){
          ...
          // for the FF produtor bug workaround (see select.js,
          // insertNewlineAtProdutor).
