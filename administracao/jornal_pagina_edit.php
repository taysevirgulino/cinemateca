<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("jornal_pagina_edit.inc.php");?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Revista Pagina: Edi&ccedil;&atilde;o </td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<form action="<?=$link_edit?>" method="post" name="frm" id="frm" enctype="multipart/form-data" onsubmit="return _Validar();">
      <table width="100%" border="0" cellspacing="3" cellpadding="0">
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdJornalEdicao">&nbsp;Revista Edição: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdJornalEdicao" class="frm" id="FrmIdJornalEdicao" style="width:500px;">
    <? for($i=0; $i < count($VObjJornalEdicao); $i++){ ?>
    <option value="<?=$VObjJornalEdicao[$i][0]?>" <? if($VObjJornalEdicao[$i][0] == $frm_id_jornal_edicao){ print("selected"); }?>><?=$VObjJornalEdicao[$i]["numero"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
        <tr>
          <td width="25%" height="25" bgcolor="#CCCCCC" class="frm_texto" id="labelFrmIdJornalEstrutura">&nbsp;Revista Estrutura: * </td>
          <td width="75%" bgcolor="#F2F2F2" style="padding:3px 3px 3px 3px">
          	  <select name="FrmIdJornalEstrutura" class="frm" id="FrmIdJornalEstrutura" style="width:500px;">
    <? for($i=0; $i < count($VObjJornalEstrutura); $i++){ ?>
    <option value="<?=$VObjJornalEstrutura[$i][0]?>" <? if($VObjJornalEstrutura[$i][0] == $frm_id_jornal_estrutura){ print("selected"); }?>><?=$VObjJornalEstrutura[$i]["nome"]?></option>
    <? } ?>
  </select>
          </td>
        </tr>
    ...
    <script language="javascript">document.getElementById("FrmIdJornalEdicao").focus();</script>
    ...
    <!--
        ...
        var a_fields = {
            'FrmIdJornalEdicao':{'l':' Revista Edição','t':'labelFrmIdJornalEdicao','r':true},
            'FrmIdJornalEstrutura':{'l':' Revista Estrutura','t':'labelFrmIdJornalEstrutura','r':true},
            'FrmImagemFile':{'l':' Imagem','t':'labelFrmImagem','r':true},
