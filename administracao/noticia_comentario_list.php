<? require_once("config.inc.php"); require_once("logon.inc.php"); require_once("noticia_comentario_list.inc.php"); ?>
<?
	require_once("layout_sup.php");
?>
<table width="100%" border="0" align="right" cellpadding="0" cellspacing="0">
  <tr>
    <td height="25" class="texto_titulo"> Notícia Comentário: Listagem </td>
  </tr>
  <tr>
  ...
  <form id="frmOperacao" name="frmOperacao" method="post" action="noticia_comentario_operation.php">
  <tr>
    <td><table width="100%" border="0" cellspacing="1" cellpadding="0" class="table_list">
      <tr bgcolor="#CCCCCC" class="tabela_titulo">
        <td height="20" width="30%"><a href="<?="?busca-comparacao=$busca_comparacao&chave=$chave&busca-campo=$busca_campo&ordenacao-campo=id_noticia&ordenacao-ordem=$ordenacao_ordem_inverso&pg=$PG"?>" title="Ordernar por: Notícia">Notícia</a></td>
        ...
        <td width="10%" height="20" bgcolor="#CCCCCC" align="center">Op&ccedil;&atilde;o</td>
      </tr>
      <?
      	$cont=0;
      ...
      ?>
      <tr>
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="left"><?	$value = NoticiaManage::buscarNoticia(1, $vobj[$i]["id_noticia"]);	print $value["titulo"]; ?></td>
        ...
        <td bgcolor="<?=$bgcolor?>" class="tabela_texto" align="center"><input type="checkbox" name="CheckBoxIdOperacao<?=$i?>" id="CheckBoxIdOperacao<?=$i?>" value="<?=$vobj[$i]["id_noticia_comentario"]?>" class="InputOperacao" /></td>
        <td bgcolor="<?=$bgcolor?>" align="center" style="padding:2px;"><a href="noticia_comentario_view.php?id=<?=$vobj[$i]["id_noticia_comentario"]?>"><img src="images/botao/Search.gif" width="18" height="18" border="0" alt="Visualizar | Código: <?=$vobj[$i]["id_noticia_comentario"]?>" /></a>&nbsp;<a href="noticia_comentario_edit.php?id=<?=$vobj[$i]["id_noticia_comentario"]?>"><img src="images/botao/Edit.gif" alt="Editar | Código: <?=$vobj[$i]["id_noticia_comentario"]?>" width="18" height="18" border="0"></a><a href="noticia_comentario_remove.php?id=<?=$vobj[$i]["id_noticia_comentario"]?>" onclick="javascript: return confirm('Deseja realmente EXCLUIR?');"><img src="images/botao/Delete.gif" alt="Excluir | Código: <?=$vobj[$i]["id_noticia_comentario"]?>" width="18" height="18" border="0"></a></td>
      </tr>
      <?
      	} 
		if($cont <= 0){
