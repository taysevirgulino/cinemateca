<?
	...
	if( !Validacao::isVazio($frm_nome) ){
		...
		if( count($error) <= 0 ){
		    ...
		    if( $obj->alterar() ){     
		        ...
		        if( $rs ){
		            System::Redirect( Url::_Path()."lojista-pessoa-list?msg-success=".base64_encode("Contato atualizado com sucesso.") );
		        }else{
		            System::Redirect( Url::_Path()."lojista-pessoa-list?msg-danger=".base64_encode("Contato foi atualizado, mas houve um erro no acesso ao site.") );
		        }
	...
	*/
	...
	$fileTemplate = "lojista_pessoa_edit.tpl.php";
	
	if( !$template->isCached($fileTemplate) ){
		/*
		 * DEFAULT
		 */
		$Titulo = "Editar Contato de Lojista";
		...
		if( !$objUsuario->buscarAttribute(UsuarioAttribute::IdUsuario(), $obj->getIdUsuario()) ){
		    $objUsuario = null;
		}
		$template->assign("objUsuarioPessoa", $objUsuario);
